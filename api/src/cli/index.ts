 ...
 */
function needsExtensions(argv: string[]): boolean {
...
export async function createCli(
	...
	securityCommand
		...
		.action(async () => {
			const { default: secretGenerate } = await import(
				'./commands/security/secret.js'
			);
	...
	dbCommand
		...
		.action(async () => {
			const { default: dbInstall } = await import(
				'./commands/database/install.js'
			);
	...
	const migrate = async (direction: 'latest' | 'up' | 'down') => {
	...
	rolesCommand
		...
		.action(async (options: { role: string; admin: boolean; app: boolean }) => {
	...
	program
		...
		.action(async (options: { skipAdminInit?: boolean }) => {

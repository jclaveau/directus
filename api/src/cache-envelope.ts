...
export function serializeCacheEnvelope<Value>(
...
export function deserializeCacheEnvelope<Value>(
	raw: string,
): DeserializedData<Value> {
	if (!raw.startsWith(ENVELOPE_HEAD)) {
		return deserializeLegacyEnvelope(raw);
	}
	...
	if (typeof parsed.base64 === 'string') {
		return {
			value: Buffer.from(parsed.base64, 'base64') as Value,
 ...
 */
function deserializeLegacyEnvelope<Value>(raw: string): DeserializedData<Value> {

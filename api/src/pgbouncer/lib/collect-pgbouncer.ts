 ...
 * argued from.
 */
export const PGBOUNCER_LIMIT_KEYS = [
	'pool_mode',
	'default_pool_size',
	'min_pool_size',
	'reserve_pool_size',
...
function durationMs(
	...
	microseconds: string | number | null | undefined,
): number {
	return count(seconds) * 1000 + count(microseconds) / 1000;
}

function flag(value: string | number | null | undefined): boolean {
	return count(value) === 1;
}

/** A size of zero means the database inherits the global default, not "none". */
function sizeOrInherited(value: string | number | null | undefined): number | null {
...
function isFronted(row: AdminRow): boolean {
 ...
 */
function isFrontedDatabase(row: AdminRow): boolean {
...
export function buildPools(
	...
	poolRows: AdminRow[],
	databaseRows: AdminRow[],
): PgBouncerPool[] {
	const configured = databaseRows.filter(isFrontedDatabase);
	const used = poolRows.filter(isFronted);

	const connectionsOf = (database: string) => {
		return endpoint.connections
			.filter((connection) => connection.database === database)
			.map((connection) => connection.name);
	};

	const pools = used.map((row): PgBouncerPool => {
	...
	for (const definition of configured) {
		...
		if (pools.some((pool) => pool.database === database)) {
			continue;
		}
...
export function buildClients(rows: AdminRow[]): PgBouncerClient[] {
...
export function buildServers(rows: AdminRow[]): PgBouncerServer[] {
	return rows.filter(isFronted).map((row) => {
...
export function buildStats(rows: AdminRow[]): PgBouncerStats[] {
	return rows.filter(isFronted).map((row) => {
		return {
			database: text(row['database']),
...
export function buildLimits(rows: AdminRow[]): PgBouncerLimit[] {
	const byKey = new Map(rows.map((row) => [text(row['key']), row]));

	return PGBOUNCER_LIMIT_KEYS.flatMap((key): PgBouncerLimit[] => {
...
async function readInstance(
...
): Promise<PgBouncerInstance> {
	const instance: PgBouncerInstance = {
		id: endpoint.id,
		host: endpoint.host,
		port: endpoint.port,
	...
	try {
		...
		if (details.includes('pools')) {
			instance.pools = buildPools(
				endpoint,
		...
		if (details.includes('stats')) {
			instance.stats = buildStats(await showPgBouncer(endpoint, 'SHOW STATS'));
		}

		if (details.includes('limits')) {
			instance.limits = buildLimits(await showPgBouncer(endpoint, 'SHOW CONFIG'));
		}

		if (details.includes('clients')) {
			instance.clients = buildClients(
				await showPgBouncer(endpoint, 'SHOW CLIENTS'),
			);
		}

		if (details.includes('servers')) {
			instance.servers = buildServers(
				await showPgBouncer(endpoint, 'SHOW SERVERS'),
			);
		}
	}
	catch (error: any) {
		// One unreachable pooler reports why it is unreachable; it does not take
		// the rest of the fleet's readings down with it.
...
export async function collectPgBouncer(
	details: PgBouncerDetail[],
): Promise<PgBouncerReport> {
	const endpoints = resolvePgBouncerEndpoints();

	const instances = await Promise.all(
		endpoints.map((endpoint) => readInstance(endpoint, details)),
	);

...
/**
 * Build scoped cache tags from the distinct scope values present across `rows`. With `requireAll` a
 * row missing any scoped cache field makes the values unresolvable (returns `null`), so the caller
 * falls back to a coarse purge rather than leaving a slice stale — used on reads (a field that wasn't
	...
	implements AbstractService<Item>
{
	...
	async readByQuery(query: Query, opts?: QueryOptions): Promise<WithMeta<Item[]>> {
		...
		// Scope this read for cache purging. The root collection gets scoped cache tags built from the
		// scope values present in the returned rows, so one owner's/partition's later write drops
		...
		if (scopedCachePurgeEnabled()) {
			...
			const rootScopedCacheTags =
				scopedCacheFields.length > 0 ? scopedCacheTagsFromRows(this.collection, scopedCacheFields, filteredRecords, true) : null;

			for (const collection of collectionsInFieldMap(fieldMapFromAst(ast, this.schema))) {
				if (collection === this.collection && rootScopedCacheTags && rootScopedCacheTags.length > 0) {

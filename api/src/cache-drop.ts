...
function redisBackedCacheStore(cache: Keyv): RedisBackedCacheStore | null {
	...
	if (typeof store?.createKeyPrefix !== 'function') {
		return null;
	}
	...
	return store as RedisBackedCacheStore;
}
...
export async function dropCacheEntries(
	...
	if (store === null) {
		...
		// Only an explicit `false` is evidence the key was absent: a store that
		// answers `undefined` has told us nothing about whether it held one.

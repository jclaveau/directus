...
import { setScopedCachePinsHeader } from './scoped-cache-pins-header.js';
...
describe('setScopedCachePinsHeader', () => {
	test('emits every pin when they fit the budget', () => {
		env['CACHE_TAGS_HEADER_MAX_SIZE'] = '1kb';
		const res = makeRes();

		setScopedCachePinsHeader(res, NAME, ['a:b=1', 'a:b=2']);

		expect(res.setHeader).toHaveBeenCalledTimes(1);
		expect(res.setHeader).toHaveBeenCalledWith(NAME, 'a:b=1, a:b=2');
	});

	test('keeps whole pins within the budget and counts the rest', () => {
		env['CACHE_TAGS_HEADER_MAX_SIZE'] = '12b';
		const res = makeRes();

		setScopedCachePinsHeader(res, NAME, ['a:b=1', 'a:b=2', 'a:b=3', 'a:b=4']);
	...
	test('a budget exactly met keeps the last pin', () => {
	...
	test('skips the pins header when not even the first pin fits', () => {
	...
	test('budgets the percent-encoded size, not the raw one', () => {
		...
		setScopedCachePinsHeader(res, NAME, ['a:b=é', 'a:b=1']);
	...
	test.each([
		...
		setScopedCachePinsHeader(res, NAME, ['a:b=1', 'a:b=é']);

		expect(res.setHeader).toHaveBeenCalledTimes(1);
		expect(res.setHeader).toHaveBeenCalledWith(NAME, 'a:b=1, a:b=%C3%A9');
	});

	test('emits nothing for no pins', () => {
		const res = makeRes();

		setScopedCachePinsHeader(res, NAME, []);
	...
	test('keeps a pin whose value holds the separator whole', () => {
		env['CACHE_TAGS_HEADER_MAX_SIZE'] = '5b';
		const res = makeRes();

		setScopedCachePinsHeader(res, NAME, ['a:b=x, y', 'c']);
	...
	test('a clamped header never makes res.setHeader throw', () => {
		...
		expect(() => setScopedCachePinsHeader(res, NAME, labels)).not.toThrow();

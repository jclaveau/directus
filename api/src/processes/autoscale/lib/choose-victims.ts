/** A worker a release could stop, and what is known about it. */
export interface ReleaseCandidate {
	pmId: number;
	/** Requests the worker is serving, or `null` where it has not said. */
...
export function chooseVictims(
	...
	return ranked.slice(0, Math.max(count, 0)).map((worker) => worker.pmId);
}

function rank(worker: ReleaseCandidate): number {
	if (worker.inFlight === null) {
		return 1;
	}

	// Fewest requests first, and every reporting worker still ranks against
	// every other: a pool where nothing is idle is released at its quietest
	// worker rather than at its oldest.

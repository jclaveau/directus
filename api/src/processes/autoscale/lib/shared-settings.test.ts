...
test('refuses a prototype key as a field the configuration lacks', () => {
	expect(() => parseSharedSettingsPatch({ constructor: 8 }))
...
test('refuses a value of the wrong shape', () => {
	expect(() => parseSharedSettingsPatch({ maxWorkers: '8' }))
		.toThrowError("'maxWorkers' has to be a number of zero or more");

	expect(() => parseSharedSettingsPatch({ maxWorkers: -1 }))
		.toThrowError("'maxWorkers' has to be a number of zero or more");

	expect(() => parseSharedSettingsPatch({ strategy: 'aggressive' }))
		.toThrowError("'strategy' has to be one of scalabus, legacy");

	expect(() => parseSharedSettingsPatch({ enabled: 'false' }))
...
test('takes a value the loop will clamp', () => {
	expect(parseSharedSettingsPatch({ maxWorkers: 10_000, note: 'incident' }))
		.toEqual({ maxWorkers: 10_000, note: 'incident' });
});

test('lays a patch over the stored shared settings', () => {
	expect(applySharedSettingsPatch(
		{ maxWorkers: 8, note: 'spike' },
		{ minWorkers: 2 },
	))
...
// A page reading shared settings of nothing but their own note would show a
// deployment as carrying some while every value it runs on comes from its
// environment.
test('drops shared settings left holding only their note', () => {
	const stamped = { maxWorkers: 8, setBy: 'jean', setFrom: 'admin' };

	expect(applySharedSettingsPatch(stamped, { maxWorkers: 8 }))
		.toEqual(stamped);

	expect(applySharedSettingsPatch(stamped, { maxWorkers: null }))
		.toBeNull();
});
...
test('takes the surface a change came in through', () => {
	expect(parseSharedSettingsPatch({ maxWorkers: 8, setFrom: 'mcp' }))

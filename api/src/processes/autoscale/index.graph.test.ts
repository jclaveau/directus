...
/**
 * Which packages the autoscaler's graph reaches.
 *
 * The factory is the proof: vitest runs it the first time any module in the
 * graph imports the package, and the flag it leaves says the process would have
 * evaluated the whole `@directus/utils` bundle — joi, date-fns, the system-data
 * tables — for a coercion `@directus/utils/values` carries alone. The original
 * is handed back so the graph loads as it does in production and every importer
 * in it is exercised.
 */
...
it('reaches the utils bundle nowhere in its graph', async () => {

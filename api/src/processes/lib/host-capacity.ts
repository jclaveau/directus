import type { ProcessHostCapacity } from '@directus/types';
 ...
 */
const UNCAPPED_V1_MEMORY = 0x7fff_ffff_ffff_f000;

async function readLimit(path: string): Promise<string | null> {
 ...
 */
async function memoryLimit(): Promise<number | null> {
	const version2 = await readLimit('/sys/fs/cgroup/memory.max');

	if (version2 !== null && version2 !== 'max') {
		return Number(version2) || null;
	}

	const version1 = await readLimit(
	...
	if (version1 !== null) {
		...
		if (bytes > 0 && bytes < UNCAPPED_V1_MEMORY) {
			return bytes;
		}
	}
 ...
 */
async function cpuLimit(): Promise<number | null> {
	const version2 = await readLimit('/sys/fs/cgroup/cpu.max');

	if (version2 !== null) {
		const [quota, period] = version2.split(/\s+/);

		if (quota !== undefined && quota !== 'max' && Number(period) > 0) {
			return Number(quota) / Number(period) || null;
		}
	}
...
let measured: Promise<ProcessHostCapacity> | null = null;
 ...
 */
export async function hostCapacity(): Promise<ProcessHostCapacity> {
	measured ??= Promise.all([memoryLimit(), cpuLimit()])
		.then(([memoryBytes, cpuCores]) => ({ memoryBytes, cpuCores }));

	return measured;
}

/** Drops the memo so a test can measure a different container. */
export function forgetHostCapacity(): void {
	measured = null;
}

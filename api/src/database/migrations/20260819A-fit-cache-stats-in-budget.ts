 ...
 */
async function isCacheStatsHypertable(
	knex: Knex,
	table: string,
): Promise<boolean> {
...
export async function up(knex: Knex): Promise<void> {
	...
	for (const table of cacheStatsHypertables) {
		if (!(await isCacheStatsHypertable(knex, table))) {
			continue;
		}
...
export async function down(knex: Knex): Promise<void> {
	for (const table of cacheStatsHypertables) {
		...
		if (table === 'directus_cache_purges') {
			await knex.raw(
		...
		await knex.raw(
			`SELECT add_compression_policy('${table}', INTERVAL '2 days')`,
		);
	}
}

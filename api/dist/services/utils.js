...
import { listCacheAuditRuns, readCacheAuditFindings, readCacheAuditRun, runCacheAudit } from "../cache-audit-runs.js";
...
import { cacheAuditScheduleState, refreshCacheAuditScheduleOverride } from "../schedules/cache-audit.js";
...
var UtilsService = class {
	...
	async sort(collection, { item, to }) {
		...
		if (shouldClearCache(cache, void 0, collection)) await flushResponseCache(cache);
	...
	*/
	async readCacheEntry(redisKey) {
		...
		const verifiedAt = filledAt === null ? null : Math.max(filledAt, auditedAt ?? 0);
		const { cache } = getCache();
		if (!cache) return {
			exists: false,
			value: null,
		...
		const tags = storedScopedCacheTagLabels(await getCacheValue(cache, cacheTagsKey(redisKey)));
		let sizes = null;
		if (value !== void 0) {
	...
	*/
	...
	async getCacheAudit(id, page = {}) {
		this.assertAdmin("inspect the cache audits");
		const parsed = typeof id === "number" || typeof id === "string" ? Number(id) : NaN;
		if (!Number.isInteger(parsed) || parsed < 1) throw new InvalidPayloadError({ reason: `'${String(id)}' is not an audit id` });
		...
		if (run === null) throw new ForbiddenError();
		return {
			...run,
			...await readCacheAuditFindings(parsed, value)
		};
	}
	/** The cron in force for the audit, where it came from, and when it next fires. */
	async getCacheAuditSchedule() {
	...
	*/
	async updateCacheAuditSchedule(rule) {
		this.assertAdmin("change the cache audit schedule");
		if (rule !== null && typeof rule !== "string") throw new InvalidPayloadError({ reason: "`rule` has to be a cron rule, or null to clear the override" });

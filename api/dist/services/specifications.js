...
var OASSpecsService = class {
	...
	async generatePaths(schema, permissions, tags) {
		...
		for (const tag of tags) if ("x-collection" in tag === false || isSystemCollection(tag["x-collection"])) {
			for (const [path, pathItem] of Object.entries(spec.paths)) for (const [method, operation] of Object.entries(pathItem)) if (operation.tags?.includes(tag.name)) {

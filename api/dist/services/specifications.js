...
var OASSpecsService = class {
	...
	async generate(host) {
		...
		const servedTagNames = new Set(Object.values(paths).flatMap((pathItem) => Object.values(pathItem)).flatMap((operation) => operation?.tags ?? []));
		const servedTags = tags?.filter((tag) => {
			return "x-collection" in tag || servedTagNames.has(tag.name);
		});
		const url = env["PUBLIC_URL"] === "/" && host ? host : env["PUBLIC_URL"];
		const spec$1 = {
			openapi: "3.0.1",
			info: {
		...
		if (servedTags) spec$1.tags = servedTags;
	...
	async generateTags(schema) {
		...
		for (const systemTag of systemTags) {
			...
			if (!systemTag["x-collection"]) tags.push(systemTag);
		}
		for (const collection of collections) if (isSystemCollection(collection.collection)) {
			for (const tag of spec.tags) if (tag["x-collection"] === collection.collection) {
				tags.push(tag);
				break;
			}
		} else {
			...
			if (collection.note) tag.description = collection.note;
			tags.push(tag);
		}
	...
	async generatePaths(schema, permissions, tags) {
		...
		for (const tag of tags) if ("x-collection" in tag === false || isSystemCollection(tag["x-collection"])) for (const [path, pathItem] of Object.entries(spec.paths)) {
			const enabledBy = pathItem["x-enabled-by"];
			if (typeof enabledBy === "string" && env[enabledBy] !== true) continue;
			for (const [method, operation] of Object.entries(pathItem)) if (operation.tags?.includes(tag.name)) {
				if (!paths[path]) paths[path] = {};
				if (this.accountability?.admin === true || "x-collection" in tag === false || !!permissions.find((permission) => permission.collection === tag["x-collection"] && permission.action === this.getActionForMethod(method))) if ("parameters" in pathItem) paths[path][method] = {
					...operation,
			...
			for (const method of [
				...
				"patch",
				"delete"
			]) if (this.accountability?.admin === true || !!permissions.find((permission) => permission.collection === collection && permission.action === this.getActionForMethod(method))) {
				if (!paths[`/items/${collection}`]) paths[`/items/${collection}`] = {};
				if (!paths[`/items/${collection}/{id}`]) paths[`/items/${collection}/{id}`] = {};
				if (listBase?.[method]) paths[`/items/${collection}`][method] = mergeWith(cloneDeep(listBase[method]), {
					...
					requestBody: ["get", "delete"].includes(method) ? void 0 : { content: { "application/json": { schema: { oneOf: [{
						type: "array",
						items: { $ref: `#/components/schemas/${tag.name}` }
					}, { $ref: `#/components/schemas/${tag.name}` }] } } } },
					responses: { "200": {
						description: "Successful request",
						content: method === "delete" ? void 0 : { "application/json": { schema: { properties: { data: schema.collections[collection]?.singleton ? { $ref: `#/components/schemas/${tag.name}` } : {
							type: "array",
							items: { $ref: `#/components/schemas/${tag.name}` }
						} } } } }
					} }
				...
				if (detailBase?.[method]) paths[`/items/${collection}/{id}`][method] = mergeWith(cloneDeep(detailBase[method]), {
					...
					requestBody: ["get", "delete"].includes(method) ? void 0 : { content: { "application/json": { schema: { $ref: `#/components/schemas/${tag.name}` } } } },
					responses: { "200": { content: method === "delete" ? void 0 : { "application/json": { schema: { properties: { data: { $ref: `#/components/schemas/${tag.name}` } } } } } } }
				}, (obj, src) => {
					if (Array.isArray(obj)) return obj.concat(src);
				});
			}
	...
	async generateComponents(schema, tags) {
		...
		for (const [name, schema$1] of Object.entries(spec.components?.schemas ?? {})) if (requiredSchemas.includes(name)) {
		...
		for (const collection of collections) {
			...
			if (isSystem) {
				...
				for (const field of fieldsInCollection) schemaComponent.properties[field.field] = cloneDeep(spec.components.schemas[tag.name].properties[field.field]) || this.generateField(schema, collection.collection, field, tags);
				...
				for (const field of fieldsInCollection) {
					...
					if (field.nullable === false && field.defaultValue === null && field.generated === false) requiredFields.push(field.field);
				}
				if (requiredFields.length > 0) schemaComponent.required = requiredFields;
				components.schemas[tag.name] = schemaComponent;
			}
		}
		return components;
	...
	generateField(schema, collection, field, tags) {
		...
		if (field.note) propertyObject.description = field.note;
		const relation = getRelation(schema.relations, collection, field.field);
		if (!relation) propertyObject = {
			...propertyObject,
			...
			if (relationType === "m2o") {
				...
				if (!relatedTag || !relation.related_collection || relation.related_collection in schema.collections === false) return propertyObject;
				...
				if (!relatedTag || !relation.related_collection || relation.collection in schema.collections === false) return propertyObject;

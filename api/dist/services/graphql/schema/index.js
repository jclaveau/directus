...
const semaphore = new Semaphore(useEnv()["GRAPHQL_SCHEMA_GENERATION_MAX_CONCURRENT"] ?? 5);
/**
...
async function generateSchema(gql, type = "schema") {
	...
	return semaphore.runExclusive(async () => {
		...
		if (!gql.accountability || gql.accountability.admin) schema = {
		...
		if (gql.scope === "system") injectSystemResolvers(gql, schemaComposer, CollectionTypes, schema);
		...
		if (readableCollections.length > 0) schemaComposer.Query.addFields(readableCollections.reduce((acc, collection) => {
		...
		if (Object.keys(schema.update.collections).length > 0) schemaComposer.Mutation.addFields(Object.values(schema.update.collections).filter((collection) => collection.collection in UpdateCollectionTypes).filter(scopeFilter).filter((collection) => READ_ONLY.includes(collection.collection) === false).reduce((acc, collection) => {
			const collectionName = gql.scope === "items" ? collection.collection : collection.collection.substring(9);
			if (collection.singleton) acc[`update_${collectionName}`] = UpdateCollectionTypes[collection.collection].getResolver(`update_${collection.collection}`);
			else {
		...
		if (Object.keys(schema.delete.collections).length > 0) schemaComposer.Mutation.addFields(Object.values(schema.delete.collections).filter((collection) => collection.singleton === false).filter(scopeFilter).filter((collection) => READ_ONLY.includes(collection.collection) === false).reduce((acc, collection) => {
			const collectionName = gql.scope === "items" ? collection.collection : collection.collection.substring(9);

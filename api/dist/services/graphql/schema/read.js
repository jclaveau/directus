...
async function getReadableTypes(gql, schemaComposer, schema, inconsistentFields) {
	...
	for (const collection of Object.values(schema.read.collections)) {
		...
		ReadableCollectionFilterTypes[collection.collection] = schemaComposer.createInputTC({
			...
			fields: Object.values(collection.fields).reduce((acc, field) => {
				...
				if (field.type === "date") acc[`${field.field}_func`] = { type: DateFunctionFilterOperators };
				if (field.type === "time") acc[`${field.field}_func`] = { type: TimeFunctionFilterOperators };
				if (field.type === "dateTime" || field.type === "timestamp") acc[`${field.field}_func`] = { type: DateTimeFunctionFilterOperators };
				if (field.type === "json" || field.type === "alias") acc[`${field.field}_func`] = { type: CountFunctionFilterOperators };
				return acc;
			}, {})
		});
		...
		AggregatedFields[collection.collection] = schemaComposer.createObjectTC({
			...
			fields: Object.values(collection.fields).reduce((acc, field) => {
				switch (getGraphQLType(field.type, field.special)) {
					case GraphQLBigInt:
					case GraphQLInt:
					case GraphQLFloat:
						acc[field.field] = {
		...
		if (Object.values(collection.fields).some((field) => {
			const graphqlType = getGraphQLType(field.type, field.special);
			if (graphqlType === GraphQLInt || graphqlType === GraphQLFloat) return true;
			return false;
		...
		if (collection.singleton === false) resolver.args = {
		...
		else resolver.args = { version: GraphQLString };
		...
		if (collection.singleton === false) ReadCollectionTypes[collection.collection].addResolver({
		...
		if (gql.scope === "items") VersionCollectionTypes[collection.collection].addResolver({
	...
	for (const relation of schema.read.relations) if (relation.related_collection) {

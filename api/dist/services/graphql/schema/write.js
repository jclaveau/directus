...
function getWritableTypes(gql, schemaComposer, schema, inconsistentFields, ReadCollectionTypes) {
	...
	for (const collection of Object.values(schema.create.collections)) {
		...
		if (Object.keys(creatableFields).length > 0) {
			...
			if (collectionIsReadable) resolverDefinition.args = ReadCollectionTypes[collection.collection].getResolver(collection.collection).getArgs();
	...
	for (const collection of Object.values(schema.update.collections)) {
		...
		if (Object.keys(updatableFields).length > 0) if (collection.singleton) UpdateCollectionTypes[collection.collection].addResolver({

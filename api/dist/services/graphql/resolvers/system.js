...
function injectSystemResolvers(gql, schemaComposer, { CreateCollectionTypes, ReadCollectionTypes, UpdateCollectionTypes }, schema) {
	...
	if (env["OPENAPI_ENABLED"] !== false) schemaComposer.Query.addFields({ server_specs_oas: {
		type: GraphQLJSON,
		resolve: dedupeResolver(async () => {
			return await new SpecificationService({
				schema: gql.schema,
	...
	if (env["GRAPHQL_INTROSPECTION"] !== false) schemaComposer.Query.addFields({ server_specs_graphql: {
		type: GraphQLString,
		...
		resolve: dedupeResolver(async (_, args) => {
			return await generateSchema(new GraphQLService({
				schema: gql.schema,
				accountability: gql.accountability,
	...
	if ("directus_fields" in schema.read.collections) {
		...
		schemaComposer.Query.addFields({
			fields: {
				...
				resolve: dedupeResolver(async () => {
					return await new FieldsService({
						accountability: gql.accountability,
						schema: gql.schema
					}).readAll();
				}, "directus_fields")
			...
			fields_by_name: {
				...
				resolve: dedupeResolver(async (_, args) => {
					return await new FieldsService({
						accountability: gql.accountability,
						schema: gql.schema
					}).readOne(args["collection"], args["field"]);
				})
			}
		});
	}
	if ("directus_relations" in schema.read.collections) {
		...
		schemaComposer.Query.addFields({
			relations: {
				...
				resolve: dedupeResolver(async () => {
					return await new RelationsService({
						accountability: gql.accountability,
						schema: gql.schema
					}).readAll();
				}, "directus_relations")
	...
	if ("directus_users" in schema.read.collections) schemaComposer.Query.addFields({ users_me: {
	...
	if ("directus_permissions" in schema.read.collections) schemaComposer.Query.addFields({ permissions_me: {
		...
		resolve: dedupeResolver(async (_, _args, __, _info) => {
			...
			return await fetchAccountabilityCollectionAccess(gql.accountability, {
				schema: gql.schema,
				knex: database_default()
			});
		})
	} });
	if ("directus_roles" in schema.read.collections) schemaComposer.Query.addFields({ roles_me: {
		...
		resolve: dedupeResolver(async (_, args, __, info) => {
			...
			query.limit = -1;
			return await service.readMany(gql.accountability.roles, query);
		})
	} });
	if ("directus_policies" in schema.read.collections) schemaComposer.Query.addFields({ policies_me_globals: {
		...
		resolve: dedupeResolver(async (_, _args, __, _info) => {
			if (!gql.accountability?.user && !gql.accountability?.role) return null;
			return await fetchAccountabilityPolicyGlobals(gql.accountability, {
				schema: gql.schema,
				knex: database_default()
			});
		})
	} });
	if ("directus_users" in schema.update.collections && gql.accountability?.user) schemaComposer.Mutation.addFields({ update_users_me: {
		type: ReadCollectionTypes["directus_users"],
	...
	if ("directus_files" in schema.create.collections) schemaComposer.Mutation.addFields({ import_file: {
	...
	if ("directus_users" in schema.create.collections) schemaComposer.Mutation.addFields({ users_invite: {
		type: GraphQLBoolean,
		args: {
		...
		resolve: async (_, args) => {
			await new UsersService({
				accountability: gql.accountability,
				schema: gql.schema
			}).inviteUser(args["email"], args["role"], args["invite_url"] || null);
			return true;
		}
	} });

	...
	schemaComposer.Mutation.addFields({
		...
		auth_refresh: {
			...
			resolve: async (_, args, { req, res }) => {
				...
				if (mode === "json") currentRefreshToken = args["refresh_token"];
				else if (mode === "cookie") currentRefreshToken = req?.cookies[env["REFRESH_TOKEN_COOKIE_NAME"]];
				else if (mode === "session") {
					const token = req?.cookies[env["SESSION_COOKIE_NAME"]];
					if (isDirectusJWT(token)) currentRefreshToken = verifyAccessJWT(token, getSecret()).session;
				}
				if (!currentRefreshToken) throw new InvalidPayloadError({ reason: `The refresh token is required in either the payload or cookie` });
		...
		auth_logout: {
			...
			resolve: async (_, args, { req, res }) => {
				...
				if (req?.cookies[env["REFRESH_TOKEN_COOKIE_NAME"]]) res?.clearCookie(env["REFRESH_TOKEN_COOKIE_NAME"], REFRESH_COOKIE_OPTIONS);
				if (req?.cookies[env["SESSION_COOKIE_NAME"]]) res?.clearCookie(env["SESSION_COOKIE_NAME"], SESSION_COOKIE_OPTIONS);
				return true;
			}
		},
		auth_password_request: {
			...
			resolve: async (_, args, { req }) => {
				...
				try {
					...
					if (isDirectusError(err, ErrorCode.InvalidPayload)) throw err;
				}
				return true;
			}
		},
		...
		users_me_tfa_generate: {
			...
			resolve: async (_, args) => {
				...
				await new AuthenticationService({
					accountability: gql.accountability,
					schema: gql.schema
				}).verifyPassword(gql.accountability.user, args["password"]);
		...
		users_me_tfa_enable: {
			...
			resolve: async (_, args) => {
				...
				await new TFAService({
					accountability: gql.accountability,
					schema: gql.schema
				}).enableTFA(gql.accountability.user, args["otp"], args["secret"]);
				return true;
			}
		},
		users_me_tfa_disable: {
			...
			resolve: async (_, args) => {
				...
				if (await service.verifyOTP(gql.accountability.user, args["otp"]) === false) throw new InvalidPayloadError({ reason: `"otp" is invalid` });
		...
		utils_random_string: {
			...
			resolve: async (_, args) => {
				...
				if (args["length"] !== void 0 && (args["length"] < 1 || args["length"] > 500)) throw new InvalidPayloadError({ reason: `"length" must be between 1 and 500` });
		...
		utils_cache_clear: {
			type: GraphQLVoid,
			resolve: async () => {
				if (gql.accountability?.admin !== true) throw new ForbiddenError();
		...
		users_invite_accept: {
			...
			resolve: async (_, args) => {
				await new UsersService({
					accountability: gql.accountability,
					schema: gql.schema
				}).acceptInvite(args["token"], args["password"]);
				return true;
			}
		},
		users_register: {
			...
			resolve: async (_, args, { req }) => {
				...
				if (ip) await rateLimiter.consume(ip);

...
function resolveSystemAdmin(gql, schema, schemaComposer) {
	if (!gql.accountability?.admin) return;
	...
	schemaComposer.Mutation.addFields({
		create_collections_item: {
			type: Collection,
			args: { data: toInputObjectType(Collection, { postfix: "_input" }).addFields({ fields: [toInputObjectType(Field, { postfix: "_input" }).NonNull] }).NonNull },
	...
	schemaComposer.Mutation.addFields({
		...
		delete_fields_item: {
			...
			resolve: async (_, args) => {
				await new FieldsService({
					accountability: gql.accountability,
					schema: gql.schema
				}).deleteField(args["collection"], args["field"]);
	...
	schemaComposer.Mutation.addFields({
		...
		delete_relations_item: {
			...
			resolve: async (_, args) => {
				await new RelationsService({
					accountability: gql.accountability,
					schema: gql.schema
				}).deleteOne(args["collection"], args["field"]);
				return {
	...
	schemaComposer.Query.addFields({ extensions: {
		...
		resolve: async () => {
			return await new ExtensionsService({
				accountability: gql.accountability,
				schema: gql.schema
			}).readAll();
		}

...
var AuthenticationService = class {
	...
	*/
	async touchLastAccess(userId) {
		await this.knex("directus_users").update({ last_access: /* @__PURE__ */ new Date() }).where({ id: userId });
		if (!scopedCachePurgeEnabled()) return;
		await createScopedCacheExtensionHandle(async () => this.schema).purgeForMutatedRows("directus_users", [{ id: userId }]);
	}
	/**
	...
	*/
	...
	async refresh(refreshToken, options) {
		...
		if (record.user_id) await this.touchLastAccess(record.user_id);

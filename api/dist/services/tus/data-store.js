...
var TusDataStore = class extends DataStore {
	...
	async create(upload) {
		...
		if (isReplacement === true && upload.metadata["id"]) upload.metadata["replace_id"] = upload.metadata["id"];
		const fileData = {
		...
		try {
			...
			logger.warn(err);
			if (isReplacement) await filesItemsService.updateOne(primaryKey, {
				tus_id: null,
				tus_data: null
			}, { emitEvents: false });
			else await filesItemsService.deleteOne(primaryKey, { emitEvents: false });
	...
	async write(readable, tus_id, offset) {
		...
		try {
			...
			if (Number(fileData.filesize) === newOffset) {
				...
				if (Boolean(fileData.tus_data?.["metadata"]?.["replace_id"]) === true) {
					...
					for await (const partPath of this.storageDriver.list(replaceId)) await this.storageDriver.delete(partPath);

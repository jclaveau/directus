...
import { assign, clone, cloneDeep, isEqual, omit, pick, reduce, without } from "lodash-es";
...
var ItemsService = class ItemsService {
	...
	async createOne(data, opts = {}) {
		...
		*/
		const primaryKey = await transaction(this.knex, async (trx) => {
			...
			if (payloadAfterHooks === null || typeof payloadAfterHooks === "string" || typeof payloadAfterHooks === "number") return payloadAfterHooks;
			...
			try {
				let dbQuery = trx.insert(payloadWithoutAliases).into(this.collection).returning(primaryKeyField).then((result$1) => result$1[0]);
				...
				const filteredResult = await emitter_default.emitFilter(["items.db.inserted", `${this.collection}.db.inserted`], result, {
					collection: this.collection,
					payload
				}, {
					database: trx,
				...
				const returnedKey = typeof filteredResult === "object" ? filteredResult[primaryKeyField] : filteredResult;
	...
	*/
	...
	isMariaDbStore;
	async isMariaDb() {
		if (this.isMariaDbStore !== void 0) return this.isMariaDbStore;
		const { version } = await this.knex.select(this.knex.raw("VERSION() as version")).first();
		return this.isMariaDbStore = version.split("-").includes("MariaDB");
	}
	async createManyAtOnce(data, opts = {}) {
		...
		const fields = Object.keys(this.schema.collections[this.collection].fields);
		const aliases = Object.values(this.schema.collections[this.collection].fields).filter((field) => field.alias === true).map((field) => field.field);
		const isPrimaryKey = (it) => {
			return typeof it === "number" || typeof it === "string";
		};
		const isNotPrimaryKey = (it) => {
		...
		const itemsValues = await this.knex.transaction(async (trx) => {
			...
			try {
				let dbQuery = trx.batchInsert(this.collection, itemsToInsert.map((v) => {
					return {
				...
				if (results.length === itemsToInsert.length) (await emitter_default.emitFilter(["items.db.inserted", `${this.collection}.db.inserted`], results, {
					collection: this.collection,
					payload: data
				}, {
					database: trx,
					...
					if (!preparedValues) throw new Error(`No batchInsert itemInput found for index ${index}`);
					...
					const itemsToInsertWithoutPk = itemsToInsert.filter((item) => {
						return item.primaryKey === void 0;
					});
					const itemsToInsertWithPk = itemsToInsert.filter((item) => {
						return item.primaryKey !== void 0 && item.primaryKey !== null;
					});
					const lastRows = await trx.select(primaryKeyField, ...inputKeys).from(this.collection).orderBy(primaryKeyField, "desc").limit(itemsToInsertWithoutPk.length);
					...
					const filteredLastAddedRows = await emitter_default.emitFilter(["items.db.inserted", `${this.collection}.db.inserted`], lastAddedRows, {
						collection: this.collection,
						payload: data
					}, {
						database: trx,
					...
					const emptyItemInput = Object.fromEntries(inputKeys.map((key) => [key, void 0]));
					...
					filteredLastAddedRows.map((addedRow, index) => {
						...
						if (item === void 0) throw new Error(`Missing item to insert at ${itemIndex} during primary keys attribution for MySql / MariaDB / SQlite`);
						if (!["mssql"].includes(this.knex.client.config.client)) {
							const sqlifyEntries = (input) => {
								const castedEntries = Object.entries(input).reduce((acc, entry) => {
									const field = collectionSchema?.fields[entry[0]];
									if (!field) throw new Error(`Field ${entry[0]} missing in the schema of the collection ${this.collection}`);
									let value;
									if (entry[1] === null || entry[1] === void 0) entry[1] = field.defaultValue;
									if (entry[1] === null || entry[1] === void 0) value = null;
									else if ("boolean" === field.type) if (entry[1] === true) value = 1;
									else if (entry[1] === false) value = 0;
									else throw new Error(`Invalid valid for boolean input: ${JSON.stringify(entry[1])}`);
									else if ([
										"string",
										"text",
										"csv",
										"uuid",
										"time"
									].includes(field.type)) value = entry[1].toString();
									else if (["json"].includes(field.type)) if (typeof entry[1] === "string") value = isMariaDb ? entry[1] : JSON.parse(entry[1]);
									else throw new Error(`Bad input value for JSON field: ${String(entry[1])}`);
									else if ([
										"timestamp",
										"date",
										"dateTime"
									].includes(field.type)) if (entry[1] instanceof Date) if (field.type === "timestamp" || field.type === "dateTime") if (isMariaDb) value = /* @__PURE__ */ new Date(Math.floor(entry[1].getTime() / 1e3) * 1e3);
									else value = /* @__PURE__ */ new Date(Math.round(entry[1].getTime() / 1e3) * 1e3);
									else if (field.type === "date") {
										value = new Date(entry[1]);
										value.setHours(0, 0, 0, 0);
									} else value = entry[1];
									else throw new Error(`Unimplemented support for date/time input '${entry[0]}' having schema type '${field.type}' and value type ${typeof entry[1]}`);
									else if ([
										"integer",
										"float",
										"bigInteger"
									].includes(field.type)) value = Number(entry[1]);
									else if (["decimal"].includes(field.type)) value = Number(entry[1]).toFixed(5);
									else if (field.type === "alias") throw new Error("Alias fields should already be removed here");
									else throw new Error(`Unimplemented support for input entry '${entry[0]}' having schema type '${field.type}' and value type ${typeof entry[1]}`);
							...
							if (!isEqual(sqlifiedItemInput, addedRow)) {
								function objectDiff(obj1, obj2) {
									return reduce(obj1, (result, value, key) => {
										if (!isEqual(value, obj2[key])) result[key] = {
											new: obj2[key],
											old: value
										};
										return result;
								...
								throw new Error(`Invalid primary key assignment of added row ${JSON.stringify(addedRow, null, 2)} to inserted item ${JSON.stringify(sqlifiedItemInput, null, 2)} differing by ${JSON.stringify(objectDiff(sqlifiedItemInput, addedRow), null, 2)}`);
							}
						}
					});
					...
					if (insertedItemsWithoutPk.length !== 0) throw new Error(`Remaining inserted items with no primary key: ${JSON.stringify(insertedItemsWithoutPk, null, 2)}`);
				}
			} catch (err) {

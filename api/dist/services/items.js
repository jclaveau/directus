...
var ItemsService = class ItemsService {
	...
	*/
	...
	async deleteMany(keys, opts = {}) {
		...
		const oldScopedCacheTags = await this.scopedCache.snapshot(keys, { deleting: true });
		if (this.accountability) await validateAccess({
			accountability: this.accountability,
			action: "delete",
			collection: this.collection,
		...
		if (shouldClearCache(this.cache, opts, this.collection)) this.scopedCachePurged = await this.scopedCache.purge(oldScopedCacheTags, scopedCacheCollector, scopedCacheCollectionsChangedByOnDelete(this.schema, this.collection));
		if (opts.emitEvents !== false) await emitActionEvents([{

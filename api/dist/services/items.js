...
import { composeScopedCachePaths, createScopedCacheCollector, pinnedScopedCacheTagsFromFilter, pinnedScopedCacheTagsFromKeyedFilters, pinnedScopedCacheTagsFromM2oParents, pinnedScopedCacheTagsFromO2mChildren, purgeScopedCache, resolveScopedCacheM2oJoinChainFromPath, scopedCacheCollectionsBeyondNestedRows, scopedCacheCollectionsChangedByOnDelete, scopedCacheFilterKeyingByCollection, scopedCacheNestedCollections, scopedCacheOwnershipNestedPkPaths, scopedCachePurgeEnabled, scopedCacheTagKey, scopedCacheTagsFromRows } from "../scoped-cache.js";
...
var ItemsService = class ItemsService {
	...
	*/
	async snapshotScopedCacheTags(keys) {
		...
		for (const { field } of paths) {
			const pathTags = await this.snapshotScopedCachePathTags(field, keys, fieldTypes[field]);
			tags.push(...pathTags);
		}
		return tags;
	}
	...
	*/
	async snapshotScopedCachePathTags(path, keys, terminalType) {
		...
		const rows = await query.select(this.knex.ref(`${prevAlias}.${resolved.terminalField}`).as("value")).whereIn(`root.${primaryKeyField}`, keys);
		return scopedCacheTagsFromRows(this.collection, [path], rows.map((row) => ({ [path]: row.value })), "skip", { [path]: terminalType });
	}
	/**
	...
	*/
	...
	async purgeScopedCache(tags, collector, changedCollections = []) {
		...
		if (ownTags !== null && otherCollections.length === 0) {
			this.scopedCachePurged = await purgeScopedCache(this.cache, this.collection, [...ownTags, ...hookTags], context);
			return;
		}
	...
	async readByQuery(query, opts) {
		...
		const records = await runAst(ast, this.schema, this.accountability, {
			...
			onRowsWithTemporaryFields: (rows) => {
				...
				o2mChildPins = pinnedScopedCacheTagsFromO2mChildren(this.schema, this.collection, fieldMap, toArray$1(rows), beyondNestedRows);
			}
		});

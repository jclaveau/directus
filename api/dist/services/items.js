...
import { createScopedCacheCollector, takenOverScopedCacheKey } from "../scoped-cache/tags.js";
import { foldHandedOverScopedCacheEpochs, readScopedCacheEpochs } from "../scoped-cache/fill-guard.js";
import { requestedFieldNestsPast } from "../scoped-cache/paths.js";
...
var ItemsService = class ItemsService {
		...
		const { nestedActionEvents, actionPayloads } = await transaction(this.knex, async (trx) => {
			...
			for (const [index, payloadInput] of data.entries()) {
				...
				if (typeof payloadAfterHooks === "string" || typeof payloadAfterHooks === "number") {
					scopedCacheCollector.takenOverKeys.add(takenOverScopedCacheKey(this.collection, payloadAfterHooks));
	...
	*/
	async readByQuery(query, opts) {
		...
		const injectedOwnershipPaths = this.scopedCache.ownershipPathsToInject(updatedQuery);
		...
		const scopedCachePlan = this.scopedCache.planRead(ast, injectedOwnershipPaths);
		...
		const { tags: scopedCacheTags, unautopurgeable: scopedCacheUnautopurgeableTags } = await this.scopedCache.readTags({
			ast,
		...
		return withMeta(filteredRecords, {
			...
			scopedCacheEpochs: foldHandedOverScopedCacheEpochs(scopedCacheEpochs, scopedCacheCollector.epochs)
		});
	}
	/**
	...
	async upsertMany(payloads, opts = {}) {
		...
		if (shouldClearCache(this.cache, opts, this.collection)) {
			...
			const scopedCacheTags = primaryKeys.some((key) => {
				return key != null && scopedCacheCollector.takenOverKeys.has(takenOverScopedCacheKey(this.collection, key));
	...
	*/
	...
	async deleteMany(keys, opts = {}) {
		...
		const oldScopedCacheTags = await this.scopedCache.snapshot([...keys, ...selfRelationSurvivorKeys]);
		if (this.accountability) await validateAccess({
			accountability: this.accountability,
			action: "delete",
			collection: this.collection,
		...
		if (shouldClearCache(this.cache, opts, this.collection)) {
			const survivorScopedCacheTags = await this.scopedCache.snapshot(selfRelationSurvivorKeys);
			const scopedCacheTags = oldScopedCacheTags === null || survivorScopedCacheTags === null ? null : [...oldScopedCacheTags, ...survivorScopedCacheTags];
			this.scopedCachePurged = await this.scopedCache.purge(scopedCacheTags, scopedCacheCollector, scopedCacheCollectionsChangedByOnDelete(this.schema, this.collection));
		}
		if (opts.emitEvents !== false) await emitActionEvents([{
...
function stripInjectedOwnershipNesting(records, injectedPaths, query, schema, rootCollection) {
	...
	const surfacesAsScalar = (prefix, field) => {
		return fields.some((requested) => {
			...
			return segments.length === prefix.length + 1 && requestedFieldNestsPast(requested, prefix) && (segments[prefix.length] === "*" || segments[prefix.length] === field);
		});
	};

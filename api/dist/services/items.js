	...
	*/
	async readByQuery(query, opts) {
		...
		if (scopedCachePurgeEnabled()) {
			...
			const collectionsFetchedAsRows = new Set([...fieldMap.read].map(([, entry]) => entry.collection));
			const ancestorSliceTagsFor = (collection) => {
				const pinsOf = (ancestor) => {
					if (ancestor === this.collection) return rootScopedCacheTags;
					return [...m2oParentPins.get(ancestor) ?? [], ...keyedFilterPins.get(ancestor) ?? []];
				};
			...
			const pushAncestorSliceOrBare = (collection) => {
				const ancestorSliceTags = o2mConflicted.has(collection) ? [] : ancestorSliceTagsFor(collection);
				scopedCacheTags.push(...ancestorSliceTags.length > 0 ? ancestorSliceTags : [{ collection }]);
			};

...
export class ItemsService {
    ...
    async createOne(data, opts = {}) {
        ...
        const primaryKey = await transaction(this.knex, async (trx) => {
            ...
            try {
                ...
                dbQuery = (await emitter.emitFilter(['items.db.insert', `${this.collection}.db.insert`], dbQuery, {
                    collection: this.collection,
                    payload,
                    ...
                    schema: this.schema,
                    accountability: this.accountability,
                })); // TODO: fix emitFilter typing
                const result = await dbQuery;
                const filteredResult = (await emitter.emitFilter(['items.db.inserted', `${this.collection}.db.inserted`], result, {
                    collection: this.collection,
                    payload,
                }, {
                    database: trx,
    ...
    async createManyAtOnce(data, opts = {}) {
        ...
        const itemsValues = await this.knex.transaction(async (trx) => {
            ...
            const sqliteFieldsRequiringValue = isSqlite ? Object.fromEntries(Object.entries(collectionFieldsSchema)
            ...
            try {
                let dbQuery = trx
                    .batchInsert(this.collection, itemsToInsert.map((v) => {
                    return {
                ...
                if (results.length === itemsToInsert.length) {
                    const filteredResult = (await emitter.emitFilter(['items.db.inserted', `${this.collection}.db.inserted`], results, {
                        collection: this.collection,
                        payload: data,
                    }, {
                        database: trx,
                    ...
                    filteredResult.forEach((result, index) => {
                        ...
                        if (this.schema.collections[this.collection].fields[primaryKeyField].type === `uuid`) {
                            preparedValues.primaryKey = getHelpers(trx).schema.formatUUID((preparedValues.primaryKey ?? returnedKey));
                        }
                        else {
                    ...
                    const filteredLastAddedRows = (await emitter.emitFilter(['items.db.inserted', `${this.collection}.db.inserted`], lastAddedRows, {
                        collection: this.collection,
                        payload: data,
                    }, {
                        database: trx,
                    ...
                    filteredLastAddedRows.map((addedRow, index) => {
                        ...
                        if (![
                            ...
                            if (!isEqual(sqlifiedItemInput, addedRow)) {
                                ...
                                // 	fieldDefaults: Object.fromEntries( Object.entries(collectionSchema.fields).map(([fieldName, field]) => [fieldName, field.defaultValue])),
                                // }, null, 2))
                                throw new Error(`Invalid primary key assignment of added row ${JSON.stringify(addedRow, null, 2)}`
    ...
    async readByQuery(query, opts) {
        ...
        const filteredRecords = opts?.emitEvents !== false
            ...
            : records;
        if (opts?.emitEvents !== false) {
            await emitter.emitAction(this.eventScope === 'items' ? ['items.read', `${this.collection}.items.read`] : `${this.eventScope}.read`, {
    ...
    async updateMany(keys, data, opts = {}) {
        ...
        if (payloadKeys.length === 0 // payloadAfterHooks == {} || null
            || (payloadKeys.length === 1
                && payloadKeys[0] === primaryKeyField) // payloadAfterHooks == {id: xxx}
        ) {
            return [];
        }

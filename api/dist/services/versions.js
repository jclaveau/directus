...
var VersionsService = class VersionsService extends ItemsService {
	...
	async save(key, data) {
		...
		if (shouldClearCache(cache, void 0, collection)) await purgeScopedCache(cache, collection, null);
		return finalVersionDelta;
	}
	async promote(version, mainHash, fields) {
		const { collection, item, delta } = withoutMeta(await this.readOne(version));

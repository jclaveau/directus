...
var ImportService = class {
	...
	async import(collection, mimetype, stream) {
		if (this.accountability?.admin !== true && isSystemCollection(collection)) throw new ForbiddenError({ reason: `'${this.accountability?.user}' can't import to '${collection}' as not being an admin` });
	...
	*/
	async exportToFile(collection, query, format, options) {
		...
		try {
			...
			const storage = toArray$1(env["STORAGE_LOCATIONS"])[0];

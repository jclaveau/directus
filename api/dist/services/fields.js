...
var FieldsService = class FieldsService {
	...
	async readAll(collection) {
		...
		if (collection) {
			...
			fields = withoutMeta(await nonAuthorizedItemsService.readByQuery({ limit: -1 }));
		...
		if (this.accountability && this.accountability.admin !== true) {
			...
			if (collection && collection in allowedFieldsInCollection === false) throw new ForbiddenError({ reason: `'${this.accountability.user}' does not have permission to read fields from '${collection}'.` });
		...
		return withMeta(result, { scopedCacheTags: [{ collection: "directus_fields" }] });
	}
	async readOne(collection, field) {
		if (this.accountability && this.accountability.admin !== true) {
			...
			if (!hasAccess) throw new ForbiddenError({ reason: `'${this.accountability.user}' does not have permission to read '${collection}.${field}'` });
		}
		let column = void 0;
		...
		if (!column && !fieldInfo) throw new ForbiddenError({ reason: `Can't retrieve the schema info for the column '${field}' of '${collection}'` });
	...
	async createField(collection, field, table, opts) {
		if (this.accountability && this.accountability.admin !== true) throw new ForbiddenError({ reason: `'${this.accountability.user}' does not have the permission to create the field '${field}' in '${collection}'` });
		...
		try {
			...
			await transaction(this.knex, async (trx) => {
				...
				if (hookAdjustedField.type && ALIAS_TYPES.includes(hookAdjustedField.type) === false) if (table) this.addColumnToTable(table, collection, hookAdjustedField);
				else await trx.schema.alterTable(collection, (table$1) => {
					this.addColumnToTable(table$1, collection, hookAdjustedField);
				});
			...
			if (shouldClearCache(this.cache, opts)) await flushResponseCache(this.cache);
	...
	async updateField(collection, field, opts) {
		if (this.accountability && this.accountability.admin !== true) throw new ForbiddenError({ reason: `'${this.accountability.user}' does not have the permission to update the field '${field}' in '${collection}'` });
		const runPostColumnChange = await this.helpers.schema.preColumnChange();
		const nestedActionEvents = [];
			...
			if (hookAdjustedField.schema) {
				...
				if (!isEqual(opts?.bypassLimits && opts.autoPurgeSystemCache === false ? sanitizeColumn(existingColumn) : existingColumn, hookAdjustedField.schema)) try {
					await transaction(this.knex, async (trx) => {
						...
						if (field.schema?.is_unique === false && existingColumn?.is_unique === true) await this.helpers.schema.dropUniqueIfExists(trx, collection, field.field);
						if (field.schema?.is_indexed === false && existingColumn?.is_indexed === true) await this.helpers.schema.dropIndexIfExists(trx, collection, field.field);
					});
				} catch (err) {
	...
	async deleteField(collection, field, opts) {
		if (this.accountability && this.accountability.admin !== true) throw new ForbiddenError({ reason: `'${this.accountability.user}' does not have the permission to delete the field '${field}' in '${collection}'` });
		const runPostColumnChange = await this.helpers.schema.preColumnChange();
		const nestedActionEvents = [];
		try {

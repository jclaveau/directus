				...
				if (payload.schema) {
					...
					*/
					...
					await trx.schema.createTable(payload.collection, (table) => {
						for (const field of payload.fields) if (field.type && ALIAS_TYPES.includes(field.type) === false) fieldsService.addColumnToTable(table, payload.collection, field, null, existingIndexes);
					});
	...
	*/
	async readOne(collectionKey) {
		...
		if (result.length === 0) throw new ForbiddenError({
			reason: `Collection '${collectionKey}' not found`,
			values: {
				accountability: this.accountability,
	...
	async deleteOne(collectionKey, opts) {
		...
		try {
			...
			if (!!collectionToBeDeleted === false) throw new ForbiddenError({
				reason: `Collection to delete '${collectionKey}' does not exist`,
				values: { collection: collectionKey }
			});
			await transaction(this.knex, async (trx) => {

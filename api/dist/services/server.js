...
import { toArray as toArray$1, toBoolean } from "@directus/utils";
...
var ServerService = class {
	...
	async serverInfo() {
		...
		if (this.accountability?.user) {
			...
			if (RESUMABLE_UPLOADS.ENABLED) info["uploads"] = { chunkSize: RESUMABLE_UPLOADS.CHUNK_SIZE };
	...
	async health() {
		...
		if (outstanding === void 0 || outstanding.length > 0) {
			data.status = "error";
		...
		async function testStorage() {
			...
			const checks = {};
			for (const location of toArray$1(env["STORAGE_LOCATIONS"])) {

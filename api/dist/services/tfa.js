...
var TFAService = class {
	...
	async generateTFA(key) {
		...
		if (!user?.email) throw new InvalidPayloadError({ reason: "User must have a valid email to enable TFA" });
	...
	async enableTFA(key, otp, secret) {
		if ((await this.knex.select("tfa_secret").from("directus_users").where({ id: key }).first())?.tfa_secret !== null) throw new InvalidPayloadError({ reason: "TFA Secret is already set for this user" });

...
import { getSimpleHash, toArray as toArray$1, validatePayload } from "@directus/utils";
import Joi from "joi";
...
var UsersService = class UsersService extends ItemsService {
	...
	*/
	async getUserByEmail(email) {
		return this.knex.select("id", "role", "status", "password", "email").from("directus_users").whereRaw(`LOWER(??) = ?`, ["email", email.toLowerCase()]).first();
	}
	/**
	...
	*/
	...
	async acceptInvite(token, password) {
		...
		if (scope !== "invite") throw new ForbiddenError({ reason: "Not an invite token" });
		const user = await this.getUserByEmail(email);
		if (user?.status !== "invited") throw new InvalidPayloadError({ reason: `Email address ${email} hasn't been invited` });
	...
	async registerUser(input) {
		...
		if (settings?.["public_registration"] == false) throw new ForbiddenError({ reason: "Public registration is disabled" });
		...
		if (emailFilter && validatePayload(emailFilter, { email: input.email }).length !== 0) {
			...
			throw new ForbiddenError({ reason: "Invalid payload" });
		}
	...
	async verifyRegistration(token) {
		...
		if (scope !== "pending-registration") throw new ForbiddenError({ reason: "Not a pending registration token" });
		const user = await this.getUserByEmail(email);
	...
	async requestPasswordReset(email, url, subject) {
		...
		if (user?.status !== "active") {
			await stall(STALL_TIME, timeStart);
			throw new ForbiddenError({ reason: "Inactive user" });
		}
	...
	async resetPassword(token, password) {
		...
		if (scope !== "password-reset" || !hash) throw new ForbiddenError({ reason: "Not a password reset token" });
		const opts = {};
		try {
		...
		const user = await this.getUserByEmail(email);
		if (user?.status !== "active") throw new ForbiddenError({ reason: "Inactive user" });
		if (hash !== getSimpleHash("" + user.password)) throw new ForbiddenError({ reason: "Bad user credentials" });
		await new UsersService({
			knex: this.knex,
			schema: this.schema,
			accountability: {

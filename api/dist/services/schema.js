...
var SchemaService = class {
	...
	async snapshot() {
		if (this.accountability?.admin !== true) throw new ForbiddenError({ reason: "Only administrators can read a schema snapshot." });
		const scopedCacheTags = [
			{ collection: "directus_collections" },
			{ collection: "directus_fields" },
			{ collection: "directus_relations" }
		];
		const scopedCacheEpochs = await readScopedCacheEpochs(scopedCacheTags.map(({ collection }) => collection));
		return withMeta(await getSnapshot({ database: this.knex }), {
			scopedCacheTags,
			scopedCacheEpochs
		});
	}
	async apply(payload) {
		if (this.accountability?.admin !== true) throw new ForbiddenError({ reason: "Only administrators can apply a schema diff." });
	...
	async diff(snapshot, options) {
		if (this.accountability?.admin !== true) throw new ForbiddenError({ reason: "Only administrators can diff the schema." });
		...
		if (diff.collections.length === 0 && diff.fields.length === 0 && diff.relations.length === 0) return null;
		return diff;
	}

...
var PermissionsService = class extends ItemsService {
	...
	async readByQuery(query, opts) {
		...
		return withMeta(withAppMinimalPermissions(this.accountability, result, query.filter), readMeta(result) ?? { scopedCacheTags: [] });
	}
	...
	async getItemPermissions(collection, primaryKey) {
		if (!this.accountability?.user) throw new ForbiddenError({ reason: "You must be authenticated to read item permissions." });

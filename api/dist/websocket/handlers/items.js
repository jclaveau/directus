...
var ItemsHandler = class {
	...
	async onMessage(client, message) {
		...
		if (!schema.collections[message.collection] || isSystemCollection(message.collection)) throw new WebSocketError("items", "INVALID_COLLECTION", "The provided collection does not exists or is not accessible.", uid);
		...
		if (message.action === "create") {
			...
			if (Array.isArray(message.data)) {
				...
				result = await service.readMany(keys.filter((key) => key !== null), query);
			} else {
		...
		if (message.action === "read") {
			const query = await sanitizeQuery(message.query ?? {}, schema, accountability);
			if (message.id) result = await service.readOne(message.id, query);
			else if (message.ids) result = await service.readMany(message.ids, query);
			else if (isSingleton) result = await service.readSingleton(query);
			else result = await service.readByQuery(query);
		...
		if (message.action === "delete") if (message.id) {
		...
		} else throw new WebSocketError("items", "INVALID_PAYLOAD", "Either 'ids', 'id' or 'query' is required for a DELETE request.", uid);

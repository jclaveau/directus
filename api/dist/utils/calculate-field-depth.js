import { isArray, isPlainObject } from "./lodash-es-used.js";
...
function calculateFieldDepth(obj, dotNotationKeys = []) {
	...
	for (const key of keys) {
		...
		if (dotNotationKeys.includes(key) && nestedValue) {
			...
			if (sortDepth > depth) depth = sortDepth;
		} else {
			if (!isPlainObject(nestedValue) && !isArray(nestedValue)) continue;
			let nestedDepth = 0;
			if (Array.isArray(nestedValue)) nestedDepth = Math.max(...nestedValue.map((val) => calculateFieldDepth(val, dotNotationKeys)));
			else nestedDepth = calculateFieldDepth(nestedValue, dotNotationKeys);
			if (key.startsWith("_") === false) nestedDepth += 1;
			if (nestedDepth > depth) depth = nestedDepth;
		}
	}
	return depth;
}

...
function resolvePreset({ transformationParams, acceptFormat }, file) {
	...
	if ((transformationParams.width || transformationParams.height) && file.width && file.height) {
		const toWidth = transformationParams.width ? Number(transformationParams.width) : void 0;
		const toHeight = transformationParams.height ? Number(transformationParams.height) : void 0;

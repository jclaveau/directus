...
function getConfigFromEnv(prefix, options) {
	...
	for (const [key, value] of Object.entries(env)) {
		...
		if (key.includes("__")) set(config, key.split("__").map((key$1, index) => index === 0 ? transform(transform(key$1.slice(prefix.length))) : transform(key$1)).join("."), value);
		else config[transform(key.slice(prefix.length))] = value;
	}
	return config;
	function transform(key) {
		if (type === "camelcase") return camelcase(key.replace(/^_+/, ""), { locale: false });

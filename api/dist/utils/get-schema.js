...
async function getDatabaseSchema(database, schemaInspector) {
	...
	const collections = [...await database.select("collection", "singleton", "note", "sort_field", "accountability", "scoped_cache_fields").from("directus_collections"), ...systemCollectionRows];
	...
	const fields = [...await database.select("id", "collection", "field", "special", "note", "validation").from("directus_fields"), ...systemFieldRows$1].filter((field) => {
		return (field.special ? toArray$1(field.special) : []).includes("no-data") === false;
	});

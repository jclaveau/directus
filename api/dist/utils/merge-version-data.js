...
function recursiveMerging(data, versionData, collection, schema) {
	...
	for (const versionRecord of versionData) {
		if (!isObject(versionRecord)) continue;
		for (const key of Object.keys(data)) {
			...
			if (key in relations === false) {
				if (isManyToAnyCollection(collection, schema) && key === "item") result[key] = recursiveMerging(addMissingKeys(isObject(currentValue) ? currentValue : {}, newValue), [newValue], data["collection"], schema);

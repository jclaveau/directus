...
function getColumnPath({ path, collection, aliasMap, relations, schema }) {
	...
	function followRelation(pathParts, parentCollection = collection, parentFields, addNestedPkField) {
		...
		*/
		...
		if (!relation) throw new InvalidQueryError({ reason: `"${parentCollection}.${pathRoot}" is not a relational field` });
		const alias = parentFields ? aliasMap[`${parentFields}.${pathParts[0]}`]?.alias : aliasMap[pathParts[0]]?.alias;
		const remainingParts = pathParts.slice(1);
		let parent;
		if (relationType === "a2o") {
			const pathScope = pathParts[0].split(":")[1];
			if (!pathScope) throw new InvalidQueryError({ reason: `You have to provide a collection scope when sorting on a many-to-any item` });
			parent = pathScope;
		} else if (relationType === "m2o") parent = relation.related_collection;
		...
		if (remainingParts.length) return followRelation(remainingParts, parent, `${parentFields ? parentFields + "." : ""}${pathParts[0]}`, addNestedPkField);
		return {
			columnPath: "",
			targetCollection: "",
			addNestedPkField

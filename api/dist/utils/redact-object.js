...
function redactObject(input, redact, replacement) {
	...
	if (redact.keys) traverse(clone, redact.keys);
	return clone;
	function traverse(object, checkKeyPaths) {
		if (checkKeyPaths.length === 0) return;
		...
		for (const key of Object.keys(object)) {
			...
			for (const [index, path] of [...checkKeyPaths].entries()) {
				...
				switch (current) {
					case escapedKey:
						if (remaining.length > 0) localCheckPaths.push(remaining);
						else {
					...
					case "*":
						if (remaining.length > 0) {
							...
							checkKeyPaths.splice(index, 1);
						} else object[key] = REDACTED_TEXT;
						break;
					case "**":
						if (remaining.length > 0) {
							const [next, ...nextRemaining] = remaining;
							if (next === escapedKey) if (nextRemaining.length === 0) object[key] = REDACTED_TEXT;
							else localCheckPaths.push(nextRemaining);
							else if (next !== void 0 && wildcardChars.includes(next)) localCheckPaths.push(remaining);
							else localCheckPaths.push(path);
						} else object[key] = REDACTED_TEXT;
						break;
				}
			}
			const value = object[key];
			if (isObject(value)) traverse(value, [...globalCheckPaths, ...localCheckPaths]);
		}
	}
}
/**
...
function getReplacer(replacement, values) {
	...
	const replacer = (seen) => {
		return function(_key, value) {
			...
			if (value !== null && typeof value === "object") {
				...
				for (const [key2, value2] of Object.entries(value)) if (typeof value2 === "string") newValue[key2] = value2;
				else newValue[key2] = replacer(seen)(key2, value2);
				seen.delete(value);
				return newValue;
			}
			if (!values || filteredValues.length === 0 || typeof value !== "string") return value;
			let finalValue = value;
			for (const [redactKey, valueToRedact] of filteredValues) if (finalValue.includes(valueToRedact)) {
	...
	return replacer(/* @__PURE__ */ new WeakSet());
}

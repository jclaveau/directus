...
async function validateDatabaseCharset(database$1) {
	...
	if (getDatabaseClient(database$1) === "mysql") {
		...
		const { version } = await database$1.select(database$1.raw("VERSION() as version")).first();
		...
		const columns = await database$1("information_schema.columns").select({
			...
			type: "COLUMN_TYPE"
		}).where({ TABLE_SCHEMA: env["DB_DATABASE"] }).whereNot({ COLLATION_NAME: collation }).modify((queryBuilder) => isMariaDB && queryBuilder.andWhereNot({

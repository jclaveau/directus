...
import database_default, { constructDatabase } from "./index.js";
import { useEnv } from "@directus/env";
import { merge } from "lodash-es";

//#region src/database/connections.ts
const namedDatabases = /* @__PURE__ */ new Map();
/**
* Name of the base pool (`DB_BASE_CONNECTION_NAME`); policies may grant it too.
*/
function getBaseConnectionName() {
	const name = useEnv()["DB_BASE_CONNECTION_NAME"];
	return typeof name === "string" ? name : "base";
}
/**
* Names of the extra connections from `DB_CONNECTIONS` (array when cast, CSV
* at runtime).
*/
function getExtraConnectionNames() {
...
function getBaseDbConfig() {
	return getConfigFromEnv("DB_", {
		omitPrefix: [
			"DB_EXCLUDE_TABLES",
			"DB_BASE_CONNECTION",
			...getExtraConnectionNames().map((name) => `DB_CONNECTION_${name.toUpperCase()}_`)
		],
		omitKey: [
...
* own knob.
*/
function getConnectionPriority(name) {
	const key = name === getBaseConnectionName() ? "DB_BASE_CONNECTION_PRIORITY" : `DB_CONNECTION_${name.toUpperCase()}_PRIORITY`;
	const priority = useEnv()[key];
	return typeof priority === "number" ? priority : 0;
}
/** Connection names must be unique (base + `DB_CONNECTIONS`); else boot fails. */
function assertConnectionNamesAreUnique() {
	const seen = new Set([getBaseConnectionName()]);
	for (const name of getExtraConnectionNames()) {
		if (seen.has(name)) throw new Error(`Duplicate DB connection name "${name}" — names must be unique`);
		seen.add(name);
	}
}
const CONNECTION_FIELD_ENV_SUFFIX = {
	client: "CLIENT",
	filename: "FILENAME",
	host: "HOST",
	port: "PORT",
...
*/
function connectionFieldEnvKey(prefix, field) {
...
*/
function requiredConnectionFields(config) {
	const fields = ["client"];
	const { client, connectionString, connectString, socketPath, type } = config;
	switch (client) {
		case "sqlite3":
			fields.push("filename");
			break;
		case "oracledb":
			if (connectString) fields.push("user", "password", "connectString");
			else fields.push("host", "port", "database", "user", "password");
			break;
		case "cockroachdb":
		case "pg":
			if (connectionString) fields.push("connectionString");
			else fields.push("host", "port", "database", "user");
			break;
		case "mysql":
			if (socketPath) fields.push("database", "user", "password", "socketPath");
			else fields.push("host", "port", "database", "user", "password");
			break;
		case "mssql":
			if (!type || type === "default") fields.push("host", "port", "database", "user", "password");
			break;
		default: fields.push("host", "port", "database", "user", "password");
	}
	return fields;
}
/**
...
*/
function resolveNamedConnectionConfig(name) {
	const { priority: _priority,...override } = getConfigFromEnv(`DB_CONNECTION_${name.toUpperCase()}_`);
...
*/
function resolveConnectionConfig(name) {
	return name === getBaseConnectionName() ? getBaseDbConfig() : resolveNamedConnectionConfig(name);
}
/**
...
*/
function assertConnectionConfigComplete(name, config) {
	const prefix = `DB_CONNECTION_${name.toUpperCase()}_`;
	for (const field of requiredConnectionFields(config)) if (config[field] === void 0 || config[field] === "") throw new Error(`DB connection "${name}" is missing "${field}" (client "${config["client"]}") — set ${connectionFieldEnvKey(prefix, field)} or inherit it from DB_*`);
}
/**
...
*/
function assertNamedConnectionsAreComplete() {
	for (const name of getExtraConnectionNames()) assertConnectionConfigComplete(name, resolveNamedConnectionConfig(name));
}
/**
...
*/
function getConnectionNameForAccountability(accountability) {
	...
	if (accountability?.share) {
		...
		if (typeof shareName === "string" && (shareName === baseName || extra.includes(shareName))) return shareName;
	}
	const candidateNames = new Set([baseName]);
	for (const name of accountability?.grantedDbConnections ?? []) if (name === baseName || extra.includes(name)) candidateNames.add(name);
	return [...candidateNames].map((name) => {
		...
		if (candidate.isBase !== winner.isBase) return winner.isBase ? candidate : winner;
		return candidate.name.localeCompare(winner.name) < 0 ? candidate : winner;
	}).name;
}
/**
* The knex a request uses (base name → base pool; named pools build lazily).
*/
function getDatabaseForAccountability(accountability) {
	const name = getConnectionNameForAccountability(accountability);
	if (name === getBaseConnectionName()) return database_default();
...
//#endregion
export { assertConnectionConfigComplete, assertConnectionNamesAreUnique, assertNamedConnectionsAreComplete, connectionFieldEnvKey, getBaseConnectionName, getBaseDbConfig, getConnectionNameForAccountability, getConnectionPriority, getDatabaseForAccountability, getExtraConnectionNames, requiredConnectionFields, resolveConnectionConfig };
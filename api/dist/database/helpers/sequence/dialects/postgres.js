...
//#region src/database/helpers/sequence/dialects/postgres.ts
var AutoIncrementHelperPostgres = class extends AutoSequenceHelper {
	/**
	...
	*/
	async resetAutoIncrementSequence(table, column) {
		return await this.knex.raw(`WITH sequence_infos AS (SELECT pg_get_serial_sequence(?, ?) AS seq_name, MAX(??) as max_val FROM ??) SELECT SETVAL(seq_name, max_val) FROM sequence_infos;`, [
			`"${table}"`,
			column,
			column,
			table
	...
	*/
	async raiseAutoIncrementSequence(table, column, providedValue) {
		return await this.knex.raw(`SELECT SETVAL(pg_get_serial_sequence(?, ?),
...
//#endregion
export { AutoIncrementHelperPostgres };
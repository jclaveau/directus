...
var SchemaHelperOracle = class extends SchemaHelper {
	...
	async dropUniqueIfExists(knex, collection, field) {
		...
		if (await this.hasIndex(knex, constraintName)) await knex.schema.alterTable(collection, (table) => {
	...
	async dropIndexIfExists(knex, collection, field) {
		...
		if (await this.hasIndex(knex, indexName)) await knex.schema.alterTable(collection, (table) => {

...
var MySQLErrorCodes = /* @__PURE__ */ function(MySQLErrorCodes$1) {
	...
	MySQLErrorCodes$1["FOREIGN_KEY_STILL_REFERENCED"] = "ER_ROW_IS_REFERENCED_2";
...
function extractError(error, data, context) {
	switch (error.code) {
		...
		case MySQLErrorCodes.FOREIGN_KEY_STILL_REFERENCED: return foreignKeyViolation(context);
	...
	function foreignKeyViolation(context$1) {
		const tickMatches = error.sqlMessage.match(/`([^`]+)`/g);
		...
		const stillReferenced = error.code === MySQLErrorCodes.FOREIGN_KEY_STILL_REFERENCED;
		const reason = stillReferenced ? "still_referenced" : "invalid_reference";
		const collection = context$1?.collection ?? childTable;
		const relatedCollection = stillReferenced ? childTable : parentTable;
		return new InvalidForeignKeyError({
			collection,
			field,
			value: field && data[field] !== void 0 ? data[field] : null,
			constraint,
			relatedCollection,
			reason,

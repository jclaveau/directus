...
async function translateDatabaseError(error, data) {
	...
	const mainThreadError = new Error(defaultError.message, { cause: defaultError });
	if (mainThreadError) mainThreadError.data = data;
	return await emitter_default.emitFilter("database.error", mainThreadError, { client }, {
		database: database_default(),
		schema: null,
		accountability: null
	});
}
async function throwDatabaseError(error, data) {
	const filteredError = await translateDatabaseError(error, data);
	if (filteredError) throw filteredError;
}

//#endregion
export { throwDatabaseError, translateDatabaseError };
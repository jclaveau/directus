...
* logging and continuing when a row or a constraint will not take.
...
async function up(knex) {
	...
	const constraintsToAdd = (await knex.select("id", "many_collection", "many_field", "one_collection").from("directus_relations")).filter((relation) => {
		return !!foreignKeys.find((fk) => fk.table === relation?.many_collection && fk.column === relation?.many_field) === false;
	});
	...
	for (const constraint of constraintsToAdd) {
		...
		if (rowsWithIllegalFKValues.length > 0) {
			...
			try {
				...
				if (ids.length < 25) logger.error(`Items with illegal foreign keys: ${ids.join(", ")}`);
				else logger.error(`Items with illegal foreign keys: ${ids.slice(0, 25).join(", ")} and ${ids.length} others`);
		...
		try {
			await knex.schema.alterTable(constraint.many_collection, (table) => {
				if (columnInfo.data_type !== relatedColumnInfo.data_type && columnInfo.data_type === "int" && relatedColumnInfo.data_type === "int unsigned") table.specificType(constraint.many_field, "int unsigned").alter();
				...
				if (constraint.many_collection !== constraint.one_collection) builder.onDelete("SET NULL");
			});
		} catch (err) {
	...
	if (corruptedRelations.length > 0) logger.warn(`Encountered one or more corrupted relationships. Please check the following rows in "directus_relations": ${corruptedRelations.join(", ")}`);
}

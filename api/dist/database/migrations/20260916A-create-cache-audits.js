...
//#region src/database/migrations/20260916A-create-cache-audits.ts
/**
...
async function up(knex) {
	await knex.schema.createTable("directus_cache_audits", (table) => {
		...
		table.integer("fresh").notNullable().defaultTo(0);
		table.integer("stale").notNullable().defaultTo(0);
		table.integer("tag_drift").notNullable().defaultTo(0);
		table.integer("raced").notNullable().defaultTo(0);
		table.integer("time_varying").notNullable().defaultTo(0);
		table.integer("expired").notNullable().defaultTo(0);
		table.integer("unreplayable").notNullable().defaultTo(0);
		table.integer("evicted").notNullable().defaultTo(0);
		table.integer("duration_ms").nullable();
		table.boolean("timed_out").notNullable().defaultTo(false);
		table.text("error").nullable();
		table.index("started_at");
	});
	await knex.schema.createTable("directus_cache_audit_findings", (table) => {
		table.increments("id");
		table.integer("audit").unsigned().notNullable().references("id").inTable("directus_cache_audits").onDelete("CASCADE");
	...
	if (getDatabaseClient(knex) === "postgres") await knex.raw(`CREATE INDEX directus_cache_stats_descriptors_audit_queue ON directus_cache_stats_descriptors ((${CACHE_ENTRY_VERIFIED_AT}), last_filled) WHERE gone_at IS NULL`);
	else await knex.schema.alterTable("directus_cache_stats_descriptors", (table) => {
		table.index(["audited_at", "last_filled"], "directus_cache_stats_descriptors_audit_queue");
	});
}
async function down(knex) {
	await knex.schema.alterTable("directus_cache_stats_descriptors", (table) => {
		table.dropIndex(["audited_at", "last_filled"], "directus_cache_stats_descriptors_audit_queue");

...
function transformStringsNewFormat(oldStrings) {
	return oldStrings.reduce((result, item) => {
		...
		for (const [language, value] of Object.entries(item.translations)) result.push({
			id: randomUUID(),
			key: item.key,
			language,
			value
...
function transformStringsOldFormat(newStrings) {
	const keyCache = {};
	for (const { key, language, value } of newStrings) set(keyCache, [key, language], value);
...
async function up(knex) {
	...
	if (data?.translation_strings && data?.id) {
		const newTranslationStrings = transformStringsNewFormat(typeof data.translation_strings === "string" ? JSON.parse(data.translation_strings) : data.translation_strings);
		for (const item of newTranslationStrings) await knex("directus_translations").insert(item);
	}

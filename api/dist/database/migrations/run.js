...
async function run(database, direction, log = true) {
	...
	if (migrations.length > migrationKeys.size) {
		const collisionsResults = migrations.reduce((acc, m) => {
			if (m.version === void 0) throw new Error("Migration version is undefined");
			if (acc.previous[m.version]) acc.collisions.push({
		...
		throw new Error("Migration keys collide! Please ensure that every migration uses a unique key." + JSON.stringify(collisionsResults.collisions, null, 2));
	}
	...
	async function up() {
		...
		if (!up$1) logger.warn(`Couldn't find the "up" function from migration ${nextVersion.file}`);
	...
	async function down() {
		...
		if (!down$1) logger.warn(`Couldn't find the "down" function from migration ${migration.file}`);
	...
	async function latest() {
		...
		for (const migration of migrations) if (migration.completed === false) {
			needsCacheFlush = true;
			const { up: up$1 } = getModuleDefault(await import(`file://${migration.file}`));
			if (!up$1) logger.warn(`Couldn't find the "up" function from migration ${migration.file}`);
		...
		if (needsCacheFlush) await flushCaches(true);
	}
}

...
async function createCli(argv = process.argv.slice(2)) {
	...
	schemaCommands.command("diff").description("Report how the current database differs from a snapshot file, or a schema-sync partial header; exits 0 when it matches, 1 when it differs").option("-q, --quiet", "Exit with the code only, without listing the changes").option("--ignoreRules <value>", "Comma-separated list of collections and or fields to ignore, as \"schema apply\" takes it").argument("<path>", "Path to snapshot file").action(async (path, options) => {
		const { default: diff } = await import("./commands/schema/diff.js");
		await diff(path, options);
	});
	program.command("edge").command("allow-list").description("Print the root paths this deployment answers on, as the allow-list an edge in front of it can enforce").addOption(new Option("--format <format>", "a Railway edge ruleset, or one path per line").choices(["railway", "plain"]).default("railway")).option("--block-status <status>", "the 4xx status the ruleset answers a blocked path with", "404").option("--include <path>", "a root segment to allow too, like /status (repeatable)", collect, []).option("--exclude <path>", "a root segment to leave out, like /admin (repeatable)", collect, []).argument("[path]", "the file to write instead of stdout, which the logs share").action(async (path, options) => {
		const { default: edgeAllowList } = await import("./commands/edge/allow-list.js");

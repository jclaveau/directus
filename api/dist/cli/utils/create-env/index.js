...
async function createEnv(client, credentials, directory) {
	...
	for (const [key, value] of Object.entries(credentials)) config["database"][`DB_${key.toUpperCase()}`] = value;
	const configAsStrings = {};
	for (const [key, value] of Object.entries(config)) {
		configAsStrings[key] = "";
		for (const [envKey, envValue] of Object.entries(value)) configAsStrings[key] += `${envKey}="${envValue}"\n`;
	}

...
function reloadBudgetMs(workers, declaration) {
	return Math.max(1, workers) * ((declaration["listen_timeout"] ?? SUPERVISOR_FALLBACKS.listenTimeout) + (declaration["kill_timeout"] ?? SUPERVISOR_FALLBACKS.killTimeout)) + RELOAD_SLACK_MS;
}
/**

//#region src/processes/autoscale/lib/supervisor.ts
/**
...
*/
function declaredNumber(value, fallback) {
	const parsed = Number(value);
	return value === void 0 || value === null || Number.isFinite(parsed) === false ? fallback : parsed;
}
/** The pm2 application declaration one worker is running under. */
function declaredBy(env) {
	const memory = Number(env.max_memory_restart);
	return {
		instances: declaredNumber(env.instances, SUPERVISOR_FALLBACKS.instances),
		execMode: typeof env.exec_mode === "string" ? env.exec_mode : SUPERVISOR_FALLBACKS.execMode,
		maxMemoryRestart: Number.isFinite(memory) && memory > 0 ? memory : null,
		listenTimeout: declaredNumber(env.listen_timeout, SUPERVISOR_FALLBACKS.listenTimeout),
		killTimeout: declaredNumber(env.kill_timeout, SUPERVISOR_FALLBACKS.killTimeout),
		minUptime: declaredNumber(env.min_uptime, SUPERVISOR_FALLBACKS.minUptime),
		maxRestarts: declaredNumber(env.max_restarts, SUPERVISOR_FALLBACKS.maxRestarts),
		restartDelay: declaredNumber(env.restart_delay, SUPERVISOR_FALLBACKS.restartDelay),
...
//#endregion
export { SUPERVISOR_FALLBACKS, declaredBy };
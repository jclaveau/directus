...
*/
function refreshMs() {
	return getMilliseconds(useEnv()["PM2_POOL_HEALTH_REFRESH"], 18e5);
}
/**
...
function poolHealthReading() {
	if (reading === null || Date.now() - readingHeardAt > 2 * refreshMs()) return null;
	return {
...
async function publishPoolHealth(health) {
	try {
		await useBus().publish(POOL_CHANNEL, health);
		return true;
	} catch (error) {
...
function reportPoolHealth(health) {
	if (withdrawn) return;
	if (reported !== null && reported.failedWorkers === health.failedWorkers && reported.onlineWorkers === health.onlineWorkers && reported.targetWorkers === health.targetWorkers && Date.now() - reportedAt < refreshMs()) return;
	reported = { ...health };
	reportedAt = Date.now();
	record(reported);
	const sent = reported;
	publishPoolHealth(sent).then((delivered) => {
		if (delivered === false && reported === sent) reported = null;
	});
}
/**
...
*/
async function answerPoolHealthQueries() {
	await useBus().subscribe(POOL_QUERY_CHANNEL, () => {
		if (reported !== null) publishPoolHealth(reported);
	});
}
/**
...
* again.
*/
async function withdrawPoolHealth() {
	withdrawn = true;
	reported = null;
...
function initPoolHealthMirror() {
	...
	const subscribed = useBus().subscribe(POOL_CHANNEL, (health) => {
		record(health);
	});
	useBus().onResubscribe(() => {
		askForReading().catch((error) => {
			useLogger().warn(error, "[pool-health] could not ask for the reading");
		});
	});
	subscribed.then(askForReading).catch((error) => {
		useLogger().warn(error, "[pool-health] no readings will be heard; health will not answer for the pool");
	});
}

//#endregion
export { answerPoolHealthQueries, initPoolHealthMirror, poolHasComeUp, poolHealthReading, reportPoolHealth, withdrawPoolHealth };
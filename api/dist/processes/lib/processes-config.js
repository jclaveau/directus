...
*/
function requestedProcessDetails(requested) {
	const reported = reportedProcessDetails();
	const asked = typeof requested === "string" ? requested.split(",") : requested;
	if (Array.isArray(asked) === false) return reported;
	const wanted = asked.map((detail) => String(detail).trim()).filter(isProcessDetail);
	return wanted.length === 0 ? reported : reported.filter((detail) => wanted.includes(detail));
}
/**
...
//#endregion
export { processesCollectTimeoutMs, processesReplicaId, processesReportEnabled, processesServiceName, reportedProcessDetails, requestedProcessDetails };
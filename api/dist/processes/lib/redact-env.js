...
//#region src/processes/lib/redact-env.ts
/**
...
*/
const SECRET_KEY_PATTERN = /PASSWORD|SECRET|TOKEN|KEY|CREDENTIAL|AUTH|SALT|PRIVATE|_URL$|_URI$|_DSN$/i;
/**
...
function isSecret(key, value) {
	if (PUBLIC_KEYS.has(key)) return false;
...
function resolveReportedEnv() {
	...
	return Object.keys(env).sort().map((key) => {
		...
		return {
			key,
			value: redacted ? null : truncate(value),
			redacted,
			isSet: value !== "",

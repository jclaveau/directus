...
function reconnectToSupervisor() {
	reconnecting ??= (async () => {
		for (const fail of outstanding) fail();
		outstanding.clear();
		await disconnected();
		await connect();
	})().finally(() => {
...
*/
function requestScale(appName, workers) {
	const supervisor = pm2;
	return new Promise((resolve, reject) => {
		const lost = () => {
			reject(/* @__PURE__ */ new Error(`the supervisor connection was rebuilt before it answered a scale to ${workers}`));
		};
...
*/
async function scaleApp(appName, workers, timeoutMs = SUPERVISOR_TIMEOUT_MS) {
	await answeredInTime(`a scale to ${workers}`, requestScale(appName, workers), timeoutMs);
}
/**
...
//#endregion
export { SUPERVISOR_TIMEOUT_MS, connectToSupervisor, disconnectFromSupervisor, listSupervisedApps, releaseWorker, reloadApp, scaleApp, sendToSupervisedProcess, supervisorAvailable, watchWorkerMessages };
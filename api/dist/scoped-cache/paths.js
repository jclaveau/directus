	...
	for (const [key, value] of Object.entries(filter)) {
		...
		if (relatedCollection !== null && hopsAcrossRelation(conditions)) {
			...
			if (nearRowKeys !== null) {
				...
				const nearRowKeyed = nearRowKeys.size > 0 && isScopedCacheKeyableField(schema, collection, fieldName);
				parts.push(new Map([[alias, nearRowKeyed ? {
					kind: "keyed",
					field: fieldName,
					keys: nearRowKeys
				} : KEYING_UNKEYED]]));
			...
			parts.push(new Map([[alias, scopePathKeying(schema, collection, fieldName, conditions) ?? KEYING_UNKEYED]]));
			continue;
		}
...
*/
function scopePathKeying(schema, collection, fieldName, conditions) {
	...
	for (;;) {
		...
		const next = node[key];
		if (!isFilterNode(next)) return null;
		segments.push(key);
		node = next;
	}
	const pinnedBy = [...(schema.collections[collection]?.scopedCacheFields ?? []).filter((scopeField) => scopeField.includes(".")), ...composeScopedCachePaths(schema, collection).map((path) => path.field)];
	const field = pinnedBy.includes(segments.join(".")) ? segments.join(".") : scopePathBeforeRelatedKey(schema, collection, segments, pinnedBy);
	if (field === null) return null;
	const keys = "_eq" in node ? new Set([node["_eq"]]) : new Set(Array.isArray(node["_in"]) ? node["_in"] : []);
	if (keys.size === 0) return null;
	const type = scopedCacheKeyedFieldType(schema, collection, field);
	if (type === void 0 || !isPinnableScopeType(type)) return null;
	return {
		kind: "keyed",
		field,
		keys
...
*/
function scopePathBeforeRelatedKey(schema, collection, segments, pinnedBy) {
	const path = segments.slice(0, -1);
	const field = path.join(".");
	if (!pinnedBy.includes(field)) return null;
	const joins = resolveScopedCacheM2oJoinChainFromPath(schema, collection, path);
	return joins?.[joins.length - 1]?.relatedPk === segments[segments.length - 1] ? field : null;
}
/**
...
*/
function scopedCacheKeyedFieldType(schema, collection, field) {
	const segments = field.split(".");
	if (segments.length === 1) return schema.collections[collection]?.fields[field]?.type;
	const joins = resolveScopedCacheM2oJoinChainFromPath(schema, collection, segments.slice(0, -1));
...
*/
function requestedFieldNestsPast(field, prefix) {
	const segments = field.split(".");
	return segments.length > prefix.length && prefix.every((segment, at) => {
		return segments[at] === "*" || segments[at] === segment;
	});
}

//#endregion
export { composeScopedCachePaths, m2oParentRowsAtPathEnd, requestedFieldNestsPast, resolveScopedCacheM2oJoinChainFromPath, scopedCacheFilterKeyingByCollection, scopedCacheKeyedFieldType, scopedCacheOwnershipNestedPkPaths };
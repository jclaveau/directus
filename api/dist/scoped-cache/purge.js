...
async function dropScopedCacheTagIndex() {
	...
	const redis = useRedis();
	for (let at = 0; at < tagKeys.length; at += SCOPED_CACHE_SWEEP_CHUNK_KEYS) await redis.del(tagKeys.slice(at, at + SCOPED_CACHE_SWEEP_CHUNK_KEYS));
}
/**
...
//#endregion
export { countScopedCacheTagMembers, dropScopedCacheTagIndex, purgeCollectionScopedCache, purgeScopedCache, retryPendingScopedCachePurges, scopedCacheCollectionsChangedByOnDelete, scopedCacheSweepScript, scopedCacheTagExpiryScript, startScopedCachePurgeRecovery, tagScopedCacheKeys };
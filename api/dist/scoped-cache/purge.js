...
import { scopedCacheIndexPrefix, scopedCacheTagKey, scopedCacheTagKeyPrefix, scopedCacheTagLabel } from "./tags.js";
...
async function tagScopedCacheKeys(key, scopedCacheTags, extraSiblings = []) {
	...
	const indexedTagKeys = /* @__PURE__ */ new Map();
	for (const tag of scopedCacheTags) {
		...
		if (ttlSeconds > 0) pipeline.scopedCacheTagExpiry(tagKey, ttlSeconds, ...members);
		...
		if (indexed === void 0) indexedTagKeys.set(slicesKey, [tagKey]);
		else indexed.push(tagKey);
	}
	for (const [slicesKey, tagKeys] of indexedTagKeys) for (let at = 0; at < tagKeys.length; at += SCOPED_CACHE_INDEX_CHUNK_MEMBERS) {
		const chunk = tagKeys.slice(at, at + SCOPED_CACHE_INDEX_CHUNK_MEMBERS);
		if (ttlSeconds > 0) pipeline.scopedCacheTagExpiry(slicesKey, ttlSeconds, ...chunk);
		else pipeline.sadd(slicesKey, ...chunk);
	}
...
*/
function sliceIndexPruningsOf(tagKeys) {
	const tagPrefix = scopedCacheTagKeyPrefix();
	const prunings = [];
	for (const tagKey of tagKeys) {
		...
		prunings.push(scopedCacheCollectionSlicesKey(label.slice(0, fieldAt)), tagKey);
	}
	return prunings;
}
/**
...
async function purgeScopedCacheTagKeys(cache, tagKeys) {
	...
	for (let at = 0; at < tagKeys.length; at += SCOPED_CACHE_SWEEP_CHUNK_KEYS) {
		...
		const swept = await redis.eval(scopedCacheSweepScript, batch.length, ...batch, ...sliceIndexPruningsOf(batch));
		for (const member of swept) if (seenMembers.has(member) === false) {
...
*/
async function unlinkScopedCacheKeysMatching(match) {
	const tally = {
		dropped: 0,
		refused: 0
	};
...
*/
async function* scanScopedCacheKeys(match) {
	const redis = useRedis();
	let cursor = "0";
	do {
		const [next, batch] = await redis.scan(cursor, "MATCH", match, "COUNT", SCOPED_CACHE_SCAN_COUNT);
		cursor = next;
		yield batch;
	} while (cursor !== "0");
}
...
async function flushResponseCache(cache) {
	...
	try {
		...
		useLogger().warn(error, `[scoped-cache] could not drop the tag index after a flush: ${error}`);
	}
}
/**
...
async function purgeCollectionScopedCache(cache, collection, options = {}) {
	...
	const sliceKeys = await useRedis().smembers(scopedCacheCollectionSlicesKey(collection));
...
* target.
*/
async function reportRecoveredScopedCacheEntries(tagKeys, reported) {
...
async function purgeScopedCache(cache, collection, scopedCacheTags = [], context = null, options = {}) {
	...
	const declaredScopedCacheTags = options.includeCollectionTag === false ? [...scopedCacheTags] : [{ collection }, ...scopedCacheTags];
	let resolvedScopedCacheTags = declaredScopedCacheTags;
	try {
		resolvedScopedCacheTags = await emitter_default.emitFilter("cache.purge", declaredScopedCacheTags, { collection }, context);
	} catch (error) {
		useLogger().warn(error, `[scoped-cache] cache.purge filter failed, purging the tags resolved without it: ${error}`);
	}
...
//#endregion
export { clearResponseCache, countScopedCacheTagMembers, dropScopedCacheIndex, flushResponseCache, purgeCollectionScopedCache, purgeScopedCache, retryPendingScopedCachePurges, scopedCacheCollectionsChangedByOnDelete, scopedCacheSweepScript, scopedCacheTagExpiryScript, startScopedCachePurgeRecovery, tagScopedCacheKeys };
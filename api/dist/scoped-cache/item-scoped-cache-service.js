...
import { composeScopedCachePaths, requestedFieldNestsPast, resolveScopedCacheM2oJoinChainFromPath, scopedCacheOwnershipNestedPkPaths } from "./paths.js";
	...
	*/
	async readTags(inputs) {
		...
		const isToOnePath = (path) => {
			return resolveScopedCacheM2oJoinChainFromPath(this.schema, this.collection, path.split(".")) !== null;
		};
		...
		const sliceTagsFor = (collection) => {
			...
			for (const slice of slicesOf(collection)) {
				...
				if (paths.every((path) => {
					return scopedCachePathReversesChain(this.schema, this.collection, path.split("."), collection, slice.segments);
		...
		const pushSliceOrBare = (collection) => {

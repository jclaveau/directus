...
async function processPayload(options, context) {
	...
	if (!options.accountability.admin) {
		...
		if (permissions.length === 0) throw new ForbiddenError({ reason: `You don't have permission to "${options.action}" from collection "${options.collection}" or it does not exist.` });
		...
		if (fieldsAllowed.includes("*") === false) {
			...
			if (notAllowed.length > 0) {
				...
				throw new ForbiddenError({ reason: notAllowed.length === 1 ? `You don't have permission to access field ${fieldStr} in collection "${options.collection}" or it does not exist.` : `You don't have permission to access fields ${fieldStr} in collection "${options.collection}" or they do not exist.` });
			}
		}

...
import { scopedCacheCollectionsWithoutGuard, scopedCacheSweptDuringFill } from "../scoped-cache/fill-guard.js";
		...
		try {
			...
			if (capturedEpochs) {
				...
				if (sweptDuringFill !== void 0) {
					if (await evictCacheEntry(cache, redisKey) === false) await recordPendingScopedCachePurge({
						mode: "slices",
						collection: req.collection ?? null,
						scopedCacheTags: scopedCacheTags.map(scopedCacheTagLabel)
					}, /* @__PURE__ */ new Error(`in-flight purge of ${sweptDuringFill} left ${redisKey} cached`));
			...
			if (env["CACHE_TAGS_HEADER"]) {
				const pins = res.locals["scopedCacheTags"];
				if (Array.isArray(pins) && pins.length) await setCacheValue(cache, cacheTagsKey(redisKey), { tags: serializeScopedCacheTags(pins) }, getMilliseconds(resolvedCacheTtl()));
			}

...
const respond = async_handler_default(async (req, res) => {
	...
	if (env["CACHE_TAGS_HEADER"]) {
		if (Array.isArray(readTags) && readTags.length) res.setHeader(`${env["CACHE_TAGS_HEADER"]}`, printableScopedCacheTags(serializeScopedCacheTags(readTags)));
	}
	...
	if (cacheableRequest && cache && exceedsMaxSize === false && orphansInScopedMode === false && unautopurgeableScope === false && unguardedScope === false && dynamicQueryFilter === false && await permissionsCachable(req.collection, {
		...
		try {
			...
			if (env["CACHE_TAGS_HEADER"]) {
				if (Array.isArray(readTags) && readTags.length) await setCacheValue(cache, cacheTagsKey(redisKey), { tags: serializeScopedCacheTags(readTags) }, getMilliseconds(resolvedCacheTtl()));
			}

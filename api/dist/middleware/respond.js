...
import { scopedCacheTagLabel } from "../scoped-cache/tags.js";
...
import { tagScopedCacheKeys } from "../scoped-cache/purge.js";
...
import { CACHE_AUDIT_TAGS_HEADER, isCacheAuditReplay } from "../utils/cache-audit-replay.js";
...
const respond = async_handler_default(async (req, res) => {
	...
	const readTags = res.locals["scopedCacheTags"] ?? payloadMeta?.scopedCacheTags;
	if (env["CACHE_TAGS_HEADER"]) {
		if (Array.isArray(readTags) && readTags.length) setScopedCacheTagsHeader(res, `${env["CACHE_TAGS_HEADER"]}`, readTags.map(scopedCacheTagLabel));
	}
	if (env["CACHE_PURGED_TAGS_HEADER"]) {
		const purged = res.locals["scopedCachePurged"];
		if (Array.isArray(purged) && purged.length) setScopedCacheTagsHeader(res, `${env["CACHE_PURGED_TAGS_HEADER"]}`, purged.map(scopedCacheTagLabel));
	}
	...
	const scopedCacheTags = readTags?.length && countsWholeCollection === false ? readTags : [...readTags ?? [], ...collectionFallbackTags];
	if (isCacheAuditReplay(req)) res.setHeader(CACHE_AUDIT_TAGS_HEADER, printableScopedCacheTags(scopedCacheTags.map(scopedCacheTagLabel).join(",")));
	const orphansInScopedMode = scopedCacheTags.length === 0 && scopedCachePurgeEnabled();
	const unautopurgeableScopeTags = res.locals["scopedCacheUnautopurgeableTags"] ?? payloadMeta?.scopedCacheUnautopurgeableTags;
	...
	const capturedEpochs = mergedScopedCacheEpochs(res.locals["scopedCacheEpochsAtRequest"], res.locals["scopedCacheEpochs"] ?? payloadMeta?.scopedCacheEpochs);
	const unguardedScopeCollections = scopedCacheCollectionsWithoutGuard(capturedEpochs, scopedCacheTags);
	const unguardedScope = unguardedScopeCollections.length > 0;
	let filled = false;
	if (cacheableRequest && cache && exceedsMaxSize === false && orphansInScopedMode === false && unautopurgeableScope === false && unguardedScope === false && dynamicQueryFilter === false && await permissionsCachable(req.collection, {
		...
		try {
			...
			await tagScopedCacheKeys(redisKey, scopedCacheTags, env["CACHE_TAGS_HEADER"] ? [cacheTagsKey(redisKey)] : []);
			...
			if (capturedEpochs) {
				const sweptDuringFill = await scopedCacheSweptDuringFill(capturedEpochs);
			...
			if (env["CACHE_TAGS_HEADER"]) {
				if (Array.isArray(readTags) && readTags.length) await setCacheValue(cache, cacheTagsKey(redisKey), { tags: readTags.map(scopedCacheTagLabel) }, getMilliseconds(resolvedCacheTtl()));
			}

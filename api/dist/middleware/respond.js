...
const respond = async_handler_default(async (req, res) => {
	...
	if ((req.method.toLowerCase() === "get" || req.originalUrl?.startsWith("/graphql")) && req.originalUrl?.startsWith("/auth") === false && env["CACHE_ENABLED"] === true && cache && !req.sanitizedQuery.export && res.locals["cache"] !== false && exceedsMaxSize === false && await permissionsCachable(req.collection, {

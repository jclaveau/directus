...
import { getConnectionNameForAccountability, getDatabaseForAccountability } from "../database/connections.js";
...
const errorHandler = asyncErrorHandler(async (err, req, res) => {
	...
	let errors = [];
	let status = null;
	const rawErrors = Array.isArray(err) ? err : [err];
	const receivedErrors = await Promise.all(rawErrors.map(async (error) => {
		let resolved = error;
		if (!isDirectusError(error)) {
			...
			try {
				...
				dialectKnex = database_default();
			}
			const translated = await extractDatabaseError(error, {}, dialectKnex);
			if (isDirectusError(translated)) resolved = translated;
		}
		if (isDirectusError(resolved, ErrorCode.DatabasePoolExhausted)) resolved.extensions.connection ??= getConnectionNameForAccountability(req.accountability);
		return resolved;
	}));
	for (const error of receivedErrors) {
		...
		const withRaw = error;
		if (getNodeEnv() === "development" && withRaw.rawDatabaseError) (error.extensions ??= {})["databaseError"] = withRaw.rawDatabaseError;
		if (isDirectusError(error)) {
			logger.debug(error);
			...
			if (isDirectusError(error, ErrorCode.DatabasePoolExhausted)) res.header("Retry-After", "1");
		} else {
			logger.error(error);

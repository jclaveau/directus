...
const checkCacheMiddleware = async_handler_default(async (req, res, next) => {
	...
	res.locals["httpRequestCacheKey"] = {
		redisKey,
		cacheKey
	};
	let cachedData;
	let expiresMeta;
	try {
		[cachedData, expiresMeta] = await getCacheValues(cache, [redisKey, cacheExpiresAtKey(redisKey)]);
	} catch (err) {
	...
	if (cachedData) {
		...
		if (env["CACHE_TAGS_HEADER"]) try {
			const labels = storedScopedCacheTagLabels(await getCacheValue(cache, cacheTagsKey(redisKey)));
			if (labels) setScopedCacheTagsHeader(res, `${env["CACHE_TAGS_HEADER"]}`, labels);
		} catch (err) {

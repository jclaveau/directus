...
import { applyOptionsData, deepMap, getRedactedString, isValidJSON, parseJSON, toArray as toArray$1 } from "@directus/utils";
...
var FlowManager = class {
	...
	constructor() {
		...
		this.envs = env["FLOWS_ENV_ALLOW_LIST"] ? pick(env, toArray$1(env["FLOWS_ENV_ALLOW_LIST"])) : {};
	...
	async load() {
		...
		for (const flow of flowTrees) if (flow.trigger === "event") {
			let events = [];
			if (flow.options?.["scope"]) events = toArray$1(flow.options["scope"]).map((scope) => {
				if ([
					"items.create",
					"items.update",
					"items.delete"
					...
					return toArray$1(flow.options["collections"]).map((collection) => {
			...
			const handler = async (data, context) => {
				...
				if (accountability.admin === false) {
					...
					if (Array.isArray(targetKeys) && targetKeys.length > 0) {
						...
						const allowedKeys = (await service.readMany(targetKeys, { fields: [primaryField] }, { emitEvents: false })).map((key) => key[primaryField]);

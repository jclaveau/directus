...
function keyedKeysAcross(parts) {
...
function keyingOfEveryCondition(parts) {
	...
	if (parts.some((part) => part.kind === "unkeyed")) return keys.size > 0 ? {
		kind: "keyed",
		keys
	} : KEYING_UNKEYED;
	if (parts.some((part) => part.kind === "keyed")) return {
		kind: "keyed",
		keys
	};
	if (parts.some((part) => part.kind === "independent")) return {
		kind: "independent",
		keys
	};
...
function keyingOfColumnConditions(schema, collection, fieldName, functionName, conditions) {
	...
	if (functionName !== void 0 || fieldName !== primaryKeyField) return KEYING_UNKEYED;
	const keyType = schema.collections[collection]?.fields[fieldName]?.type;
	if (!isPinnableScopeType(keyType)) return KEYING_UNKEYED;
...
function scopedCacheCollectionsBeyondNestedRows(schema, ast, keyingByCollection = scopedCacheFilterKeyingByCollection(schema, ast)) {
	...
	const addCollectionsQueriedBy = (collection, query, cases) => {
		...
		const sortedGroupedFieldMap = {
			read: /* @__PURE__ */ new Map(),
			other: /* @__PURE__ */ new Map()
		};
		const sortedGroupedQuery = {};
		if (query.sort) sortedGroupedQuery.sort = query.sort;
		if (query.group) sortedGroupedQuery.group = query.group;
		if (query.aggregate) sortedGroupedQuery.aggregate = query.aggregate;
		extractFieldsFromQuery(collection, sortedGroupedQuery, sortedGroupedFieldMap, schema);
		const sortedOrGrouped = /* @__PURE__ */ new Set();
		for (const [, entry] of [...sortedGroupedFieldMap.read, ...sortedGroupedFieldMap.other]) sortedOrGrouped.add(entry.collection);
		for (const [, entry] of [...queryFieldMap.read, ...queryFieldMap.other]) {
			const kind = keyingByCollection.get(entry.collection)?.kind;
			if ((kind === "keyed" || kind === "independent") && !sortedOrGrouped.has(entry.collection)) continue;
...
*/
function pinnedScopedCacheTagsFromO2mChildren(schema, rootCollection, fieldMap, records, collectionsBeyondNestedRows) {

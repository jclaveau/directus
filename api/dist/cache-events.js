...
async function readCacheDescriptorForRedisKey(redisKey) {
	...
	const db = database_default();
	const found = await db("directus_cache_stats_descriptors").where("cache_key", redisKey).first("cache_key", "last_filled", "audited_at") ?? await db("directus_cache_stats_descriptors").where("redis_key", redisKey).first("cache_key", "last_filled", "audited_at");
	if (found === void 0 || found["last_filled"] === null) return null;
	return {
		cacheKey: found["cache_key"],
		lastFilled: new Date(found["last_filled"]),
		auditedAt: found["audited_at"] === null ? null : new Date(found["audited_at"])
	};
}
/**
...
*/
async function readCacheAuditQueue(count, before, filter = {}) {
	if (!cacheStatsConfigured() || count <= 0) return [];
	const query = database_default()("directus_cache_stats_descriptors").whereNotNull("last_filled").whereNot("redis_key", "").whereNull("gone_at").whereRaw(`${CACHE_ENTRY_VERIFIED_AT} < ?`, [before]).orderByRaw(CACHE_ENTRY_VERIFIED_AT).orderBy("last_filled", "asc").limit(count).select("cache_key", "redis_key", "method", "path", "collection", "user_id", "query", "last_filled");
	if (filter.user !== void 0) query.where("user_id", filter.user);
	if (filter.collection !== void 0) query.where("collection", filter.collection);
	return (await query).map((row) => {
		return {
			...
			query: row["query"],
			lastFilled: new Date(row["last_filled"])
		};
	});
}
/**
...
*/
async function readScopedCacheEntryTags(cacheKeys) {
	const tagsByCacheKey = /* @__PURE__ */ new Map();
	if (!cacheStatsConfigured() || cacheKeys.length === 0) return tagsByCacheKey;
	const tagRows = await database_default()("directus_cache_stats_scoped_entry_tags").whereIn("cache_key", cacheKeys).select("cache_key", "scoped_cache_tag");
	for (const tagRow of tagRows) {
		const cacheKey = tagRow["cache_key"];
		const tags = tagsByCacheKey.get(cacheKey) ?? [];
		tags.push(tagRow["scoped_cache_tag"]);
...
*/
async function readCacheAuditQueueState() {
	if (!cacheStatsConfigured()) return {
		size: 0,
		neverAudited: 0,
		verifiedSince: null
	...
	const db = database_default();
	const row = await db("directus_cache_stats_descriptors").whereNotNull("last_filled").whereNot("redis_key", "").whereNull("gone_at").first(db.raw("COUNT(*) AS size"), db.raw("SUM(CASE WHEN audited_at IS NULL THEN 1 ELSE 0 END) AS never_audited"), db.raw(`MIN(${CACHE_ENTRY_VERIFIED_AT}) AS verified_since`));
	return {
		size: Number(row?.["size"] ?? 0),
		neverAudited: Number(row?.["never_audited"] ?? 0),
		verifiedSince: row?.["verified_since"] == null ? null : new Date(row["verified_since"]).getTime()
	};
}
/**
...
*/
async function advanceCacheAuditQueue(cacheKeys, at) {
	if (cacheKeys.length === 0) return;
	await database_default()("directus_cache_stats_descriptors").whereIn("cache_key", cacheKeys).update({ audited_at: at });
}
/**
...
*/
async function retireCacheAuditQueue(cacheKeys, askedAt) {
	if (cacheKeys.length === 0) return;
	await database_default()("directus_cache_stats_descriptors").whereIn("cache_key", cacheKeys).where("last_filled", "<=", askedAt).update({ gone_at: askedAt });
}
/**
...
async function listCacheEntries(windowMs) {
	...
	return rows.map((row) => {
		...
		const auditedAt = row["audited_at"] == null ? null : new Date(row["audited_at"]).getTime();
		return {
			key: row["cache_key"],
...
async function enforceCacheStatsBudget() {
	...
	let bytes$1 = await schema.getTablesSize(CACHE_STATS_TABLES);
...
//#endregion
export { CACHE_LATENCY_METRICS, CACHE_TIMESERIES_MAX_BUCKETS, CACHE_TIMESERIES_MIN_BUCKETS, advanceCacheAuditQueue, cacheStatsActive, cacheStatsConfigured, claimCacheAnomalyThrottleSlot, clampCacheStatsWindow, drainCacheEvents, effectiveTtlByBucket, enforceCacheStatsBudget, evictCacheEntriesForPath, evictCacheEntry, flushCacheEventBuffer, getCacheStatsState, listCacheAnomalies, listCacheEntries, listCacheGroupLatencies, listPurgesCoveringEntry, queueCacheAnomaly, queueCacheDescriptor, queueCacheHit, queueCacheMiss, queueCachePurge, queueMissLatency, readCacheAuditQueue, readCacheAuditQueueState, readCacheDescriptorForRedisKey, readCacheMissGap, readCacheTimeseries, readCacheTombstone, readScopedCacheEntryTags, reapCacheAnomalies, reapCacheConfigEvents, reapCacheDescriptors, reapCacheEvents, reapCachePurges, reapScopedCacheEntryTags, reapScopedCachePurgeTags, recordCacheConfigEvent, refreshCacheStatsFlag, retireCacheAuditQueue, setCacheStatsEnabled, subscribeCacheStatsToggle, truncateCacheEvents, writeCacheTombstone };
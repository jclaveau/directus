...
async function persistStreamBatch(redis, db, batch) {
	...
	try {
		await db.transaction(async (trx) => {
			if (events.length > 0) await trx.batchInsert("directus_cache_events", events, FLUSH_BATCH);
			if (descriptors.size > 0) await trx("directus_cache_descriptors").insert([...descriptors.values()]).onConflict("cache_key").merge();
			if (locators.size > 0) await trx("directus_cache_descriptors").insert([...locators.values()]).onConflict("cache_key").ignore();
			if (anomalies.length > 0) await trx.batchInsert("directus_cache_anomalies", anomalies, FLUSH_BATCH);
			if (purges.length > 0) await trx.batchInsert("directus_cache_purges", purges, FLUSH_BATCH);
			if (purgedScopedCacheTags.length > 0) await trx.batchInsert("directus_scoped_cache_purge_tags", purgedScopedCacheTags, FLUSH_BATCH);
			if (entryScopedCacheTags.size > 0) {
				await trx("directus_scoped_cache_entry_tags").whereIn("cache_key", [...entryScopedCacheTags.keys()]).delete();
				...
				if (rows.length > 0) await trx.batchInsert("directus_scoped_cache_entry_tags", rows, FLUSH_BATCH);
			}
		});
	} catch (err) {
...
function scopedCachePurgeCoverage(db, reach, cacheKeys, since) {
	if (reach === "tag") return db("directus_scoped_cache_entry_tags as et").join("directus_scoped_cache_purge_tags as pt", "pt.scoped_cache_tag", "et.scoped_cache_tag").where("pt.time", ">", since).whereIn("et.cache_key", cacheKeys);
	return db("directus_scoped_cache_purge_tags as pt").join("directus_scoped_cache_entry_tags as et", "et.collection", "pt.collection").where("pt.time", ">", since).where("pt.scoped_cache_tag", "").whereIn("et.cache_key", cacheKeys);
}
/**
...
async function listPurgesCoveringEntry(cacheKey, since) {
	...
	const db = database_default();
	const rows = [];
	for (const reach of SCOPED_CACHE_PURGE_REACHES) rows.push(...await scopedCachePurgeCoverage(db, reach, [cacheKey], since).join("directus_cache_purges as p", "p.purge_id", "pt.purge_id").distinct("p.purge_id", "p.time", "p.mode", "p.collection", "pt.scoped_cache_tag", "p.evicted").orderBy("p.time", "desc").orderBy("pt.scoped_cache_tag", "asc").limit(CACHE_ENTRY_PURGE_LIMIT));
	rows.push(...await db("directus_cache_purges as p").where("p.mode", "namespace").where("p.time", ">", since).select("p.purge_id", "p.time", "p.mode", "p.collection", "p.evicted").orderBy("p.time", "desc").limit(CACHE_ENTRY_PURGE_LIMIT));
...
async function readCacheDescriptorForRedisKey(redisKey) {
	...
	const db = database_default();
	const found = await db("directus_cache_descriptors").where("cache_key", redisKey).first("cache_key", "last_filled") ?? await db("directus_cache_descriptors").where("redis_key", redisKey).first("cache_key", "last_filled");
...
async function listCacheEntries(windowMs) {
	...
	const eventAggregateRows = await db("directus_cache_events as e").where("e.time", ">", since).whereExists((filledDescriptor) => {
		filledDescriptor.select(db.raw("1")).from("directus_cache_descriptors as d").whereRaw("?? = ??", ["d.cache_key", "e.cache_key"]).whereNotNull("d.last_filled");
	...
	const descriptorRows = listedKeys.length === 0 ? [] : await db("directus_cache_descriptors as d").leftJoin("directus_users as u", "u.id", "d.user_id").whereIn("d.cache_key", listedKeys).select("d.cache_key", "d.redis_key", "d.coarse", "d.method", "d.path", "d.collection", "d.user_id", "u.email as user_email", "d.query", "d.bytes", "d.fill_ms", "d.last_filled");
...
async function listCacheGroupLatencies(windowMs) {
	...
	const rows = await db("directus_cache_descriptors as d").join("directus_cache_events as e", "e.cache_key", "d.cache_key").where("e.time", ">", since).whereIn("e.kind", [
		0,
		2,
		3,
		4
...
async function evictCacheEntriesForPath(cache, path) {
	...
	const keys = await database_default()("directus_cache_descriptors").where({ path }).pluck("redis_key");
...
async function reapCacheEvents() {
	...
	return database_default()("directus_cache_events").where("time", "<", cutoff).delete();
}
/**
...
async function reapScopedCachePurgeTags() {
	if (!cacheStatsConfigured()) return 0;
	const cutoff = new Date(Date.now() - retentionMs());
	return database_default()("directus_scoped_cache_purge_tags").where("time", "<", cutoff).delete();
}
/**
...
async function reapCachePurges() {
	if (!cacheStatsConfigured()) return 0;
	const cutoff = new Date(Date.now() - retentionMs());
	return database_default()("directus_cache_purges").where("time", "<", cutoff).delete();
}
/**
...
async function listCacheAnomalies(windowMs) {
	...
	return (await db("directus_cache_anomalies as a").join("directus_cache_descriptors as d", "d.cache_key", "a.cache_key").where("a.time", ">", since).groupBy("a.cache_key", "a.reason", "d.path", "d.method", "d.query").select("a.cache_key", "a.reason", "d.path", "d.method", "d.query", db.raw("COUNT(*) AS count"), db.raw("MAX(a.detail) AS sample"), db.raw("MAX(a.time) AS last_seen")).orderBy("count", "desc").orderBy("a.cache_key", "asc").orderBy("a.reason", "asc").limit(CACHE_STATS_LISTING_LIMIT)).map((row) => {
		return {
...
async function reapCacheAnomalies() {
	if (!cacheStatsConfigured()) return 0;
	const cutoff = new Date(Date.now() - retentionMs());
	return database_default()("directus_cache_anomalies").where("time", "<", cutoff).delete();
}
/**
...
async function recordCacheConfigEvent(kind, detail) {
	await database_default()("directus_cache_config_events").insert({
...
async function readCacheTimeseries(windowMs, buckets = 60) {
	...
	const markers = (await db("directus_cache_config_events").where("time", ">", since).orderBy("time", "asc").select("time", "kind", "detail")).map((row) => {
		return {
			time: new Date(row["time"]).getTime(),
			kind: row["kind"],
	...
	const priorChange = await db("directus_cache_config_events").where("kind", "ttl_change").andWhere("time", "<=", since).orderBy("time", "desc").first();
	...
	const eventRows = await db("directus_cache_events").where("time", ">", since).groupByRaw("1").select(db.raw(`${bucketExpr} AS bucket`, [since, bucketSec]), db.raw("SUM(CASE WHEN kind = 0 THEN 1 ELSE 0 END) AS hits"), db.raw("SUM(CASE WHEN kind = 1 THEN 1 ELSE 0 END) AS misses"), db.raw("SUM(CASE WHEN kind = 2 THEN 1 ELSE 0 END) AS fills"), db.raw("MAX(ttl_ms) AS ttl_ms"));
	const pct = (p, filter) => {
		const base = `percentile_cont(${p}) WITHIN GROUP (ORDER BY duration_ms)`;
		return filter ? `${base} FILTER (WHERE ${filter})` : base;
	};
	const latencyRows = await db("directus_cache_events").where("time", ">", since).whereIn("kind", [
		0,
		2,
		3,
		4
	...
	const anomalyRows = await db("directus_cache_anomalies").where("time", ">", since).groupByRaw("1").select(db.raw(`${bucketExpr} AS bucket`, [since, bucketSec]), db.raw("COUNT(*) AS count"));
	const distinctPurges = db("directus_cache_purges").where("time", ">", since).distinct("purge_id", "time", "mode", "evicted", "duration_ms").as("p");
...
async function reapCacheConfigEvents() {
	const cutoff = new Date(Date.now() - retentionMs());
	return database_default()("directus_cache_config_events").where("time", "<", cutoff).delete();
}
/**

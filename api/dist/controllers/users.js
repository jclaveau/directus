...
router.post("/me/tfa/generate/", async_handler_default(async (req, res, next) => {
	if (!req.accountability?.user) throw new InvalidCredentialsError();
	const requiresPassword = (await getDatabase().select("provider").from("directus_users").where({ id: req.accountability.user }).first())?.["provider"] === DEFAULT_AUTH_PROVIDER;
	if (requiresPassword && !req.body.password) throw new InvalidPayloadError({ reason: `"password" is required` });
	const service = new TFAService({
		accountability: req.accountability,
		schema: req.schema
	});
	if (requiresPassword) await new AuthenticationService({
		accountability: req.accountability,
		schema: req.schema
...
router.post("/:pk/tfa/disable", async_handler_default(async (req, _res, next) => {
	if (!req.accountability?.user) throw new InvalidCredentialsError();
	if (!req.accountability.admin || !req.params["pk"]) throw new ForbiddenError();
	await new TFAService({
		accountability: req.accountability,
		schema: req.schema

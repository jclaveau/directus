...
function getCurrentRefreshToken(req, mode) {
	if (mode === "json") return req.body.refresh_token;
	if (mode === "cookie") return req.cookies[env["REFRESH_TOKEN_COOKIE_NAME"]];
	if (mode === "session") {
...
router.post("/logout", async_handler_default(async (req, res, next) => {
	...
	const currentRefreshToken = getCurrentRefreshToken(req, getCurrentMode(req));
	if (!currentRefreshToken) throw new InvalidPayloadError({ reason: `The refresh token is required in either the payload or cookie` });
...
router.post("/password/request", async_handler_default(async (req, _res, next) => {
	if (typeof req.body.email !== "string") throw new InvalidPayloadError({ reason: `"email" field is required` });
	const accountability = createDefaultAccountability({ ip: getIPFromReq(req) });
	const userAgent = req.get("user-agent")?.substring(0, 1024);
	if (userAgent) accountability.userAgent = userAgent;
	const origin = req.get("origin");
...
router.post("/password/reset", async_handler_default(async (req, _res, next) => {
	if (typeof req.body.token !== "string") throw new InvalidPayloadError({ reason: `"token" field is required` });
	if (typeof req.body.password !== "string") throw new InvalidPayloadError({ reason: `"password" field is required` });
	const accountability = createDefaultAccountability({ ip: getIPFromReq(req) });
	const userAgent = req.get("user-agent")?.substring(0, 1024);
	if (userAgent) accountability.userAgent = userAgent;
	const origin = req.get("origin");

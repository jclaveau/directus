...
import { systemMcpAllowsOrigin } from "../system-mcp/lib/config.js";
import { SUPPORTED_MCP_PROTOCOL_VERSIONS, handleSystemMcpRequest } from "../system-mcp/lib/handle-request.js";
import "../system-mcp/index.js";
import { ForbiddenError, InvalidPayloadError, MethodNotAllowedError } from "@directus/errors";
import { Router } from "express";

//#region src/controllers/system-mcp.ts
const router = Router();
/**
...
*/
router.post("/", async_handler_default(async (req, res) => {
	if (systemMcpAllowsOrigin(req.headers.origin) === false) throw new ForbiddenError({ reason: `Origin '${req.headers.origin}' is not named in SYSTEM_MCP_ALLOWED_ORIGINS` });
	if (req.accountability?.admin !== true) throw new ForbiddenError({ reason: "The system MCP endpoint is admin only" });
	const version = req.headers["mcp-protocol-version"];
	if (typeof version === "string" && SUPPORTED_MCP_PROTOCOL_VERSIONS.includes(version) === false) throw new InvalidPayloadError({ reason: `Unsupported MCP-Protocol-Version: ${version}` });
	...
	if (method === "tools/call") logger.info(trace, "System MCP tool call");
	else logger.debug(trace, "System MCP request");
...
var system_mcp_default = router;

//#endregion
export { system_mcp_default as default };
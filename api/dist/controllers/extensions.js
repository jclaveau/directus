...
import { isNil } from "../utils/lodash-es-used.js";
...
router.get("/registry", async_handler_default(async (req, res, next) => {
	if (req.accountability && req.accountability.admin !== true) throw new ForbiddenError({ reason: `'${req.accountability?.user}' does not have permission to access registry` });
	...
	if (filter) {
		...
		if (by) query.by = by;
		if (type) {
			if (isIn(type, EXTENSION_TYPES) === false) throw new InvalidPayloadError({ reason: `Unknown extension type "${type}"` });
			query.type = type;
		}
	}
...
router.post("/registry/install", async_handler_default(async (req, _res, next) => {
	if (req.accountability && req.accountability.admin !== true) throw new ForbiddenError({ reason: `'${req.accountability?.user}' does not have permission to access registry/install` });
	const { version, extension } = req.body;
	if (!version || !extension) throw new InvalidPayloadError({ reason: "Both \"version\" and \"extension\" are required" });
...
router.post("/registry/reinstall", async_handler_default(async (req, _res, next) => {
	if (req.accountability && req.accountability.admin !== true) throw new ForbiddenError({ reason: `'${req.accountability?.user}' does not have permission to access registry/reinstall` });
	const { extension } = req.body;
	if (!extension) throw new InvalidPayloadError({ reason: "\"extension\" is required" });
	await new ExtensionsService({
		accountability: req.accountability,
		schema: req.schema
	}).reinstall(extension);
...
router.delete(`/registry/uninstall/:pk(${UUID_REGEX})`, async_handler_default(async (req, _res, next) => {
	if (req.accountability && req.accountability.admin !== true) throw new ForbiddenError({ reason: `'${req.accountability?.user}' does not have permission to access registry/uninstall` });
	const pk = req.params["pk"];
	if (typeof pk !== "string") throw new InvalidPayloadError({ reason: "Extension primary key must be a string" });
	await new ExtensionsService({
		accountability: req.accountability,
		schema: req.schema
	}).uninstall(pk);
...
router.patch(`/:pk(${UUID_REGEX})`, async_handler_default(async (req, res, next) => {
	if (req.accountability && req.accountability.admin !== true) throw new ForbiddenError({ reason: `'${req.accountability?.user}' cannot update extension` });
	if (typeof req.params["pk"] !== "string") throw new InvalidPayloadError({ reason: "Extension primary key must be a string" });
...
router.delete(`/:pk(${UUID_REGEX})`, async_handler_default(async (req, _res, next) => {
	if (req.accountability && req.accountability.admin !== true) throw new ForbiddenError({ reason: `'${req.accountability?.user}' can't delete extension` });
	const service = new ExtensionsService({
		accountability: req.accountability,
		schema: req.schema
	});
...
router.get("/sources/:chunk", async_handler_default(async (req, res) => {
	...
	let source;
	if (chunk === "index.js") source = await extensionManager.getAppExtensionChunk();
	else source = await extensionManager.getAppExtensionChunk(chunk);

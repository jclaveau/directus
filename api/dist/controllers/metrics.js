...
router.get("/", async_handler_default(async (req, _res, next) => {
	...
	if (parts.length !== 2 || parts[0].toLowerCase() !== "metrics") throw new ForbiddenError({ reason: `Invalid authorization header` });
	if (metricTokens.find((mt) => mt.toString() === parts[1]) !== void 0) return next();
	throw new ForbiddenError({ reason: `Invalid metrics authorization token` });

...
const readHandler = async_handler_default(async (req, res, next) => {
	...
	let result;
	if (req.body.keys) result = await service.readMany(req.body.keys, req.sanitizedQuery);
...
router.get("/me/globals", async_handler_default(async (req, res, next) => {
	try {
		if (!req.accountability?.user && !req.accountability?.role) throw new ForbiddenError({ reason: `Can't access /me/globals without being logged in` });
...
router.get("/:pk", async_handler_default(async (req, res, next) => {
	...
	const record = await new PoliciesService({
		accountability: req.accountability,
		schema: req.schema

...
router.get("/me", async_handler_default(async (req, res, next) => {
	if (!req.accountability?.user && !req.accountability?.role) throw new ForbiddenError({ reason: `Can't access /me without being logged in` });
	const service = new RolesService({
		accountability: req.accountability,
		schema: req.schema
	});
...
router.get("/:pk", async_handler_default(async (req, res, next) => {
	const record = await new RolesService({
		accountability: req.accountability,
		schema: req.schema

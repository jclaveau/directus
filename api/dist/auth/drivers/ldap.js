...
var LDAPAuthDriver = class extends AuthDriver {
	...
	constructor(options, config) {
		...
		this.bindClient = ldap.createClient({
			url: clientUrl,
			reconnect: true,
			...clientConfig
		});
	...
	async validateBindClient() {
		...
		return new Promise((resolve, reject) => {
			this.bindClient.search(bindDn, {}, (err, res) => {
				...
				res.on("error", () => {
					this.bindClient.bind(bindDn, bindPassword, (err$1) => {
						if (err$1) {
							const error = handleError(err$1);
		...
		return new Promise((resolve, reject) => {
			...
			this.bindClient.search(baseDn, {
				...
				res.on("searchEntry", (entry) => {
					...
					if (Array.isArray(cn)) userGroups = [...userGroups, ...cn];
					else if (cn) userGroups.push(cn);
				});
				res.on("error", (err$1) => {
					reject(handleError(err$1));
				});
	...
	async getUserID(payload) {
		...
		if (userId) {
			...
			await this.usersService.updateOne(userId, updatedUserPayload$1);
			return userId;
		}
...
const handleError = (e) => {
	if (e instanceof ldap.InappropriateAuthenticationError || e instanceof ldap.InvalidCredentialsError || e instanceof ldap.InsufficientAccessRightsError) return new InvalidCredentialsError();
...
const ldapEntryToObject = (entry) => {
	const object = {};
	for (const { type, values } of entry.pojo.attributes) object[type] = values.length === 1 ? values[0] : values;
	return object;
};

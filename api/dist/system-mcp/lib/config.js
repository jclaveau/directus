...
function isSystemMcpToolGroup(value) {
	return value === "processes" || value === "cache" || value === "cache_audit" || value === "autoscale" || value === "autoscale_drill";
}
/**
...
function systemMcpToolGroups() {
	...
	return configured.map((group) => String(group).trim()).filter(isSystemMcpToolGroup).filter((group) => group !== "cache_audit" || cacheAuditEnabled());
}
/**

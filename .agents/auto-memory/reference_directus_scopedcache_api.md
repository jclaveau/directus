...
**Out-of-band sibling (#304/#306, MERGED):** `ApiExtensionContext.scopedCache.purgeForMutatedRows(collection, mutatedRows)` — the SAME `scopedCache` handle name on the REGISTRATION context (hook/endpoint/operation), for raw writes that bypass ItemsService. Row-based (host derives slices from `scopedCacheFields`), fail-safe to collection-wide on relational scope or a row missing a field. Register-type only. Full detail: [[project_directus_pr306_purge_for_mutated_rows]]. Delivered via context (not a host import) to avoid dragging `@directus/api` types into the extension build ([[feedback_expose_via_context_not_import]]).

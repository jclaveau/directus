...
const ENDPOINT_OVERRIDES = { directus_oauth_clients: "/mcp-oauth/clients" };
function getEndpoint(collection) {
	if (collection in ENDPOINT_OVERRIDES) return ENDPOINT_OVERRIDES[collection];
...
function parseFilterEntry([key, value], accountability, context) {
	...
	else if (String(key) === "_json") return { [key]: parseJsonFilterValue(value, accountability, context) };
...
function parseJsonFilterValue(jsonFilter, accountability, context) {
	const result = {};
	for (const [key, value] of Object.entries(jsonFilter)) if (key === "_or" || key === "_and") result[key] = value.map((subFilter) => parseJsonFilterValue(subFilter, accountability, context));
	else if ([
		"_in",
		"_nin",
		"_between",
		"_nbetween"
	].includes(key)) if (Array.isArray(value)) result[key] = value.flatMap((v) => parseDynamicVariable(v, accountability, context));
	else if (isObject(value)) result[key] = Object.values(value).flatMap((v) => parseDynamicVariable(v, accountability, context));
	else result[key] = typeof value === "string" ? parseDynamicVariable(value, accountability, context) : value;
	else if (key.startsWith("_")) result[key] = parseDynamicVariable(value, accountability, context);
	else result[key] = parseJsonFilterValue(value, accountability, context);
	return result;
}
...
const messageConstructor$2 = ({ category, duration }) => `${category} timed out after ${(0, import_ms.default)(duration, { long: true })}.`;

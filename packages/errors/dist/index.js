...
let ErrorCode = /* @__PURE__ */ function(ErrorCode$1) {
	...
	ErrorCode$1["DatabasePoolExhausted"] = "DATABASE_POOL_EXHAUSTED";
...
const messageConstructor$17 = ({ collection, field }) => `Field "${field}" in collection "${collection}" contains null values.`;
const ContainsNullValuesError = createError(ErrorCode.ContainsNullValues, messageConstructor$17, 400);
...
//#endregion
//#region src/errors/database-pool-exhausted.ts
...
const DatabasePoolExhaustedError = createError(ErrorCode.DatabasePoolExhausted, messageConstructor$16, 429);
...
const messageConstructor$13 = (extensions) => {
	const { collection, field, value, relatedCollection, reason, operation } = extensions;
	if (reason === "still_referenced") {
		if (operation === "delete" || operation === "update") {
			let message$2 = `Cannot ${operation}`;
			const hasSingleKey = value != null && value !== "" && !!field && !field.includes(",");
			if (collection && hasSingleKey) message$2 += ` "${collection}:${value}"`;
			else if (collection) message$2 += ` collection "${collection}"`;
			message$2 += ": it is still referenced";
			if (relatedCollection) message$2 += ` by collection "${relatedCollection}"`;
			return `${message$2}.`;
		}
		let message$1 = "Record";
		if (collection) message$1 += ` in collection "${collection}"`;
		message$1 += " is still referenced";
		if (relatedCollection) message$1 += ` by collection "${relatedCollection}"`;
		return `${message$1}.`;
	}
	...
	if (relatedCollection) message += ` (references "${relatedCollection}")`;
	return `${message}.`;
};

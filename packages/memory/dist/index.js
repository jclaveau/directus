...
var BusRedis = class {
	...
	compressionMinSize;
	handlers;
	resubscribeCallbacks = /* @__PURE__ */ new Set();
	readyBefore = false;
	constructor(config) {
		...
		this.sub.on("ready", () => {
			...
			if (channels.length > 0) this.sub.subscribe(...channels).then(() => {
				if (reconnected) this.resubscribeCallbacks.forEach((callback) => callback());
			}).catch((error) => {
	...
	*/
	readyCheckSettled() {
		if (this.sub.status !== "connect") return Promise.resolve();
...
var CacheMulti = class {
	...
	*/
	...
	async get(key) {
		...
		if (shared !== void 0 && this.clearGeneration === generationBeforeRead) await this.local.set(key, shared);
		return shared;
	}
	async set(key, value) {

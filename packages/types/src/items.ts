...
export type QueryOptions = {
	...
	emitEvents?: boolean;
	/**
	 * Opt a `readByQuery` out of the service-level read-through cache. Caching is
	 * on by default, but only happens when it's safe (data cache enabled, not in a
	 * transaction, not a system collection, permissions are cachable). Set `false`
	 * for a read that must always be fresh; ignored when caching wouldn't happen
	 * anyway.
	 */
	cache?: boolean;
};

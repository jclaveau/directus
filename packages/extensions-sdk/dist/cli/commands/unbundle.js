 ...
 */
export default async function unbundle(bundle, directory) {
    ...
    if (size > 200e6) {
        log(`${path.resolve(mapPath)} is ${(size / 1e6).toFixed(0)} MB;`
            + ` node may need --max-old-space-size to parse it.`, 'warn');
    }
    ...
    if (unmapped > 0) {
        log(`${unmapped} source(s) had no usable path, written under _unmapped/.`, 'warn');
    }
    if (collided > 0) {
        log(`${collided} source(s) shared a path; the later ones carry a suffix.`, 'warn');
    }
}

...
var CockroachDB = class {
	...
	async getDatabaseName() {
		if (!this.databaseName) this.databaseName = (await this.knex.raw(`SELECT current_database() AS db`)).rows[0].db;
		return this.databaseName;
	}
	async resolveTableSchema(table) {
		return (await this.knex.select("table_schema").from("information_schema.tables").whereIn("table_schema", this.explodedSchema).andWhere({
			table_name: table,
			table_type: "BASE TABLE"
		}).first())?.table_schema ?? this.explodedSchema[0];
	}
	sanitizeIdentifier(schema, name) {
		return `${this.knex.client.wrapIdentifier(schema)}.${this.knex.client.wrapIdentifier(name)}`;
	}
	/**
	...
	*/
	...
	async overview() {
      ...
      `, [this.explodedSchema.join(",")])]);
		...
		const geometryColumns = (await this.knex.raw(`WITH geometries as (
		...
		for (const column of columns) {
			...
			else column.default_value = parseDefaultValue$5(column.default_value);
	...
	*/
	async tables() {
		...
		const results = await Promise.all(this.explodedSchema.map((schema) => this.knex.raw(`SHOW TABLES FROM ${this.sanitizeIdentifier(dbName, schema)}`)));
		const tables = /* @__PURE__ */ new Set();
		for (const result of results) for (const table of result.rows) {
	...
	async tableInfo(table) {
		const dbName = await this.getDatabaseName();
		const tables = (await Promise.all(this.explodedSchema.map((schema) => this.knex.raw(`SHOW TABLES FROM ${this.sanitizeIdentifier(dbName, schema)} WITH COMMENT`)))).flatMap((r) => r.rows).filter((r) => r.type === "table");
		if (table) {
			const tableInfo = tables.find((r) => r.table_name === table);
			if (!tableInfo) throw new Error(`Table information for "${table}" not found`);
			return {
	...
	*/
	...
	extractColumnTypeMetadata(sqlType) {
		...
		if (typeof intPrecisionByType[t] === "number") return {
			max_length: null,
...
`, [this.explodedSchema, this.explodedSchema]);
		const constraintsByKey = /* @__PURE__ */ new Map();
		...
		const fetchForOneTable = async (schema, t) => {
...
GROUP BY 
	...
	c.crdb_sql_type,
    c.ordinal_position
ORDER BY c.ordinal_position`, [t, schema])).rows.map((r) => {
				...
				const defaultVal = r.column_default ? parseDefaultValue$5(r.column_default) : null;
				const hasAutoIncrement = typeof defaultVal === "string" && (defaultVal.includes("unique_rowid()") || defaultVal.startsWith("nextval("));
				const typeMetadata = this.extractColumnTypeMetadata(r.crdb_sql_type);
				const normalizedDataType = ["integer", "int"].includes(r.data_type.toLowerCase()) && typeMetadata.numeric_precision === 64 ? "bigint" : r.data_type;
				return {
					table: t,
					name: r.column_name,
		...
		if (table) {
			const cols = await fetchForOneTable(await this.resolveTableSchema(table), table);
			if (column) return cols.find((x) => x.name === column);
			return cols;
		}
		const tableInfos = await this.tableInfo();
		const flattened = (await Promise.all(tableInfos.map((t) => fetchForOneTable(t.schema, t.name)))).flat();
		if (column) return flattened.find((c) => c.name === column);
		return flattened;
	}
	/**
	...
	*/
	async primary(table) {
		return (await this.knex.select("information_schema.key_column_usage.column_name").from("information_schema.key_column_usage").leftJoin("information_schema.table_constraints", "information_schema.table_constraints.constraint_name", "information_schema.key_column_usage.constraint_name").whereIn("information_schema.table_constraints.table_schema", this.explodedSchema).andWhere({
			"information_schema.table_constraints.constraint_type": "PRIMARY KEY",
			"information_schema.table_constraints.table_name": table
		}).first()).column_name || null;
	}
...
function rawColumnToColumn$2(rawColumn) {
	...
	function parseMaxLength(rawColumn$1) {
		...
		if ([
			...
			"nchar",
			"ntext"
		].includes(rawColumn$1.data_type)) return max_length === -1 ? null : max_length / 2;
...
var MSSQL = class {
	...
	async overview() {
		...
		for (const column of columns) {
			...
			overview[column.table_name].columns[column.column_name] = {
				...column,
				default_value: column.is_identity ? "AUTO_INCREMENT" : parseDefaultValue$4(column.default_value),
	...
	*/
	...
	async columnInfo(table, column) {
		...
		const schemaId = (await this.knex.select("schema_id").from("sys.schemas").where({ name: this.schema }).first()).schema_id;
		if (!schemaId) throw new Error(`Schema '${this.schema}' not found.`);
		const dbResult = await this.knex.transaction(async (trx) => {
			...
			const query = trx.with("FilteredIndexInfo", this.knex.raw(`
				SELECT
					...
					[index_column_count]
				FROM ##IndexInfo
				WHERE ISNULL(index_column_count, 1) = 1 AND ISNULL(index_priority, 1) = 1`)).select(trx.raw(`
				[o].[name] AS [table],
				[c].[name] AS [name],
				[t].[name] AS [data_type],
				...
				[cc].[definition] as [generation_expression]`)).from(trx.raw(`??.[sys].[columns] [c]`, [dbName])).joinRaw(`JOIN [sys].[types] [t] ON [c].[user_type_id] = [t].[user_type_id]`).joinRaw(`JOIN [sys].[tables] [o] ON [o].[object_id] = [c].[object_id]`).joinRaw(`JOIN [sys].[schemas] [s] ON [s].[schema_id] = [o].[schema_id]`).joinRaw(`LEFT JOIN [sys].[computed_columns] AS [cc] ON [cc].[object_id] = [c].[object_id] AND [cc].[column_id] = [c].[column_id]`).joinRaw(`LEFT JOIN [sys].[foreign_key_columns] AS [fk] ON [fk].[parent_object_id] = [c].[object_id] AND [fk].[parent_column_id] = [c].[column_id]`).joinRaw(`LEFT JOIN FilteredIndexInfo [i] ON [i].[object_id] = [c].[object_id] AND [i].[column_id] = [c].[column_id]`).where({ "s.schema_id": schemaId });
			if (table) query.andWhere({ "o.name": table });
			if (column) query.andWhere({ "c.name": column }).first();
			const result = await query;
			await trx.raw(`DROP TABLE ##IndexInfo;`);
			return result;
		});
		if (column) return rawColumnToColumn$2(dbResult);
		return dbResult.map(rawColumnToColumn$2);
	}
	/**
	* Check if a table exists in the current schema/database
	*/
...
var MySQL = class {
	...
	async overview() {
		...
		for (const column of columns[0]) {
			...
			overview[column.table_name].columns[column.column_name] = {
				...column,
				default_value: column.extra === "auto_increment" ? "AUTO_INCREMENT" : parseDefaultValue$3(column.default_value),
				is_nullable: column.is_nullable === "YES",
...
var oracleDB = class {
	...
	`);
		...
		for (const column of columns) {
			...
			overview[column.table_name].columns[column.column_name] = {
				...
				default_value: hasAutoIncrement ? "AUTO_INCREMENT" : parseDefaultValue$2(column.default_value)
			};
		}
		return overview;
	}
	...
	*/
	async primary(table) {
		return (await this.knex.with("uc", this.knex.select(this.knex.raw(`/*+ MATERIALIZE */ "CONSTRAINT_NAME"`)).from("USER_CONSTRAINTS").where({
			TABLE_NAME: table,
			CONSTRAINT_TYPE: "P"
		})).select(this.knex.raw(`
          /*+ OPTIMIZER_FEATURES_ENABLE('${OPTIMIZER_FEATURES}') */
	...
	*/
	...
	async overview() {
      ...
      `, this.explodedSchema)]);
		...
		for (const column of columns) {
			if (column.is_identity || column.default_value?.startsWith("nextval(")) column.default_value = "AUTO_INCREMENT";
			else column.default_value = parseDefaultValue$1(column.default_value);
			if (column.table_name in overview === false) overview[column.table_name] = {
				columns: {},
				primary: void 0
			};
	...
	async overview() {
		...
		for (const table of tables) {
			...
			for (const column of columns) overview[table].columns[column.name] = {
				...
				default_value: column.pk === 1 && tablesWithAutoIncrementPrimaryKeys.includes(table) ? "AUTO_INCREMENT" : parseDefaultValue(column.dflt_value),

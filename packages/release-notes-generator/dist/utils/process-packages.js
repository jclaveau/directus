...
async function processPackages() {
    ...
    if (manualMainVersion) {
        const workspacePackage = workspacePackages.find((p) => p.manifest.name === config_js_1.default.mainPackage);
        if (workspacePackage) {
            workspacePackage.manifest.version = mainVersion;
            await workspacePackage.writeProjectManifest(workspacePackage.manifest);
            packageVersions.set(config_js_1.default.mainPackage, mainVersion);
        }
    }
    for (const [trigger, target] of config_js_1.default.linkedPackages) {
        if (packageVersions.has(trigger) && !packageVersions.has(target)) {
            ...
            if (workspacePackage && workspacePackage.manifest.version) {
                const bumpedVersion = semver_1.default.inc(workspacePackage.manifest.version, isPrerelease ? 'prerelease' : 'patch', prereleaseId);
                if (bumpedVersion) {

...
async function processPackages() {
    const workspacePackages = await (0, workspace_find_packages_1.findWorkspacePackagesNoCheck)(process.cwd());
    ...
    for (const localPackage of workspacePackages) {
        ...
        const changelogPath = (0, node_path_1.join)(localPackage.rootDir, 'CHANGELOG.md');
    ...
    async function bumpPackage(packageName, version, bumpDependents) {
        ...
        if (version) {
            ...
            newVersion = prereleaseId
                ? semver_1.default.inc(workspacePackage.manifest.version, release, prereleaseId)
                : semver_1.default.inc(workspacePackage.manifest.version, release);
        }
        if (!newVersion)
            return;

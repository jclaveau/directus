...
// (cache-config-broadcast.test.ts, autoscale-mcp-levers.test.ts). Such a tool
// writes through a plain `ItemsService('directus_settings')`, never through
// `SettingsService`, so every guarantee bolted onto that subclass is silently
// skipped — which is how a `cache_ttl` reset reached the DB in production while
// the running nodes kept serving the previous value.
//
...
//   POST /   the singleton payload to write, verbatim

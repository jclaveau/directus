...
// (jclaveau/directus#508). Here every replay of STALL is the stall — the
// `cache-audit-stall` hook blocks the loop for 300ms per read once its flag is
// armed, six times the limiter's ceiling — and the run has to come back
// fresh all the same, while the limiter is shedding anyone else.
//
...
// More than the four replays the audit holds in flight: the limiter samples
// once a second, so the first batch is through before it has seen a stall,
// and the batches after it are what a limiter left in front would shed.
const ENTRIES = 8;
const STALL_MS = 300;
...
describe('The cache audit replays through the pressure limiter', () => {
	describe.each(vendors)('%s', (vendor) => {
		...
		env[vendor]['PRESSURE_LIMITER_MAX_EVENT_LOOP_DELAY'] = '50';
		...
		it(oneLine`
			reports every replay fresh while each of them stalls the loop past the
			limiter's ceiling, and the limiter sheds anyone else meanwhile
		`, async () => {
			for (const id of ids) {
				await readRow(id);
			...
			expect(settled!.body.data).toMatchObject({ scanned: ENTRIES, fresh: ENTRIES });
			...
			// And the limiter saw them: its last sample fell after a stall, and
			// holds for the second until the next. Anyone else is shed for it.
			const bystander = await request(url)
				.get('/server/ping');

			expect(bystander.statusCode).toBe(503);
			expect(bystander.body.errors[0].extensions.reason).toBe('Under pressure');
		}, 60_000);
	});
});

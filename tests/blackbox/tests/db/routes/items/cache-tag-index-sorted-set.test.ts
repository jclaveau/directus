...
// `CACHE_SCOPED_TAG_INDEX` chooses how a numeric scope slice is filed: key-per-value
// one Redis key per distinct value, read with SMEMBERS (the default) sorted-set
// one key per (collection, field), value = score, read with ZRANGEBYSCORE — the
// whole numeric axis in a single key Nothing about which entries a write drops is
// supposed to change, so this runs the same scenario under both settings. A slice
// the sorted-set path failed to file would show as a stale HIT after a write; one it
// filed under the wrong score would show as a MISS on the untouched sibling; a key
// read with the wrong Redis type would 500. Each setting spawns its own instance
// with its own namespace: the setting is read per process, and flipping it on the
// shared instance would race every other suite.
...
describe('scoped cache tag index', () => {
	describe.each(INDEX_MODES)('CACHE_SCOPED_TAG_INDEX=%s', (indexMode) => {
		describe.each(vendors)('%s', (vendor) => {
			...
			env[vendor]['CACHE_SCOPED_TAG_INDEX'] = indexMode;
			env[vendor]['CACHE_STORE'] = 'redis';
			env[vendor]['REDIS_HOST'] = 'localhost';
			env[vendor]['REDIS_PORT'] = '6108';
			env[vendor]['CACHE_NAMESPACE'] = `directus-tag-index-${indexMode}-${vendor}`;
			...
			it('drops only the written numeric slice', async () => {
				...
				// And a MISS here would mean it found too much — every value of the field

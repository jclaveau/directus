...
const BOOT_NOTE = 'test_items_outage_boot_note';
...
// The outage above begins after boot. One that begins before it meets the stores
// while they are still dialing, and a dial node-redis retries without end is a
// promise that never settles: a wait for it, however well meant for the fresh
// process whose first command it spares, would hold every cached read of a node
// that started during an outage. Same contract as above — a read costs the cache
// and nothing else — from the one moment the case above cannot reach.
describe('the API boots with Redis unreachable', () => {
	describe.each(vendors)('%s', (vendor) => {
		const env = cloneDeep(config.envs);
		let proxy: ReturnType<typeof createRedisProxy>;
		let instance: ChildProcess;
		...
		beforeAll(async () => {
			...
			// Created and not opened: nothing listens on the port the instance is
			// given until the case says so.
			const proxyPort = await getPort();
			proxy = createRedisProxy(6108, proxyPort);
			...
			env[vendor]['REDIS_HOST'] = 'localhost';
			env[vendor]['REDIS_PORT'] = String(proxyPort);
			env[vendor]['CACHE_NAMESPACE'] = `directus-outage-boot-${vendor}`;
			env[vendor]['REDIS_RETRY_BASE_DELAY'] = '10';
			env[vendor]['REDIS_RETRY_MAX_DELAY'] = '50';
		...
		afterAll(async () => {
			...
			await proxy.cut();
			await DeleteCollection(vendor, { collection: BOOT_NOTE });
		});
		...
		it(oneLine`
			serves a read as a MISS within its budget rather than holding it on a dial
			that never answers, and caches once Redis arrives
		`, async () => {
			const askedAt = Date.now();

			const served = await get(`/items/${BOOT_NOTE}/${note}`)
				.catch(async (error: Error) => {
					await assertInstanceAlive();
					throw error;
				});
			...
			mark(`caching once up status=${status}`);
			expect(status).toBe('HIT');
			await assertInstanceAlive();
		}, 120_000);
	});

...
describe('Seed Database Structure', async () => {
	...
	// Clear Redis cache before seeding (ported from directus/directus@main).
	// Without this, stale cache entries from a prior Directus boot can short-circuit
	// collection-creation reads in the seed phase, causing the per-vendor `it.each`
	// to register zero tests (observed on Oracle).

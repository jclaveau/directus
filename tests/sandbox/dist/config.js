...
const directusConfig = {
	...
	TELEMETRY: "false",
	CONFIG_PATH: join(directusFolder, ".env"),
...
async function getEnv(database, opts) {
	const portMap = {};
	...
	const env = {
		...
		...opts.extras.license ? {
			...
			LICENSE_API_URL: `http://${base.HOST}:$PORT_LICENSE`
		} : {},
		PORT: String(opts.port),
		PUBLIC_URL: `http://${base.HOST}:${opts.port}`
	};
	if (opts.dbVersion && "DB_VERSION" in env) env.DB_VERSION = opts.dbVersion;
	for (let [key, value] of Object.entries(env)) {
		...
		for (const match of matches ?? []) {
			...
			if (match in portMap === false) portMap[match] = String(await getPort(opts.docker.port));
